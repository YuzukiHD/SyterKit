...
static int load_sdcard(image_info_t *image) {
	...
	if (ret) printk_info("FATFS: Splash load fail, Leave Black Screen.\n");
	else
...
static inline char to_hex_char(uint8_t value) { return (value < 10) ? ('0' + value) : ('A' + value - 10); }
...
_add_dts_size:
	...
	if (data.dtbo != NULL) {
		...
		if (!fdt_check_header(image->of_overlay_dest)) {
			...
			if (ret) { printk_warning("dtb overlay not success applied, overlay not applied.\n"); }
		}
	}
