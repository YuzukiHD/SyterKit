...
TM_INLINE void tm_dot_prod(mtype_t *sptr, mtype_t *kptr, uint32_t size, sumtype_t *result) {
	...
	for (; i < size; i++) { sum += sptr[i] * kptr[i]; }
	*result = sum;
	return;
}
...
TM_INLINE void tm_dot_prod_gap_3x3x1(mtype_t *sptr, mtype_t *kptr, uint32_t *k_oft, sumtype_t *result) {
	*result = sptr[k_oft[0]] * kptr[0] + sptr[k_oft[1]] * kptr[1] + sptr[k_oft[2]] * kptr[2] + sptr[k_oft[3]] * kptr[3] + sptr[k_oft[4]] * kptr[4] + sptr[k_oft[5]] * kptr[5] +
...
TM_INLINE void tm_dot_prod_3x3x1(mtype_t *sptr, mtype_t *kptr, sumtype_t *result) {
	*result = sptr[0] * kptr[0] + sptr[1] * kptr[1] + sptr[2] * kptr[2] + sptr[3] * kptr[3] + sptr[4] * kptr[4] + sptr[5] * kptr[5] + sptr[6] * kptr[6] + sptr[7] * kptr[7] +
			  sptr[8] * kptr[8];
	return;
}
...
TM_INLINE void tm_postprocess_sum(sumtype_t sum, btype_t b, int act, mtype_t *outp, sctype_t scale, sctype_t out_s, zptype_t out_zp) {//printf("sum=%.6f,", sum);
...
TM_INLINE void tm_postprocess_sum(int n, sumtype_t *sums, btype_t *bs, int act, mtype_t *outp, sctype_t *scales, sctype_t out_s, zptype_t out_zp) {
	for (int i = 0; i < n; i++) {
