...
tm_err_t TM_WEAK tm_preprocess(tm_mdl_t *mdl, tm_pp_t pp_type, tm_mat_t *in, tm_mat_t *out) {
	...
	switch (pp_type) {
		...
		case TMPP_FP2INT:
			for (int i = 0; i < in_size; i++) out->data[i] = (mtype_t) (in->dataf[i] / in_s + in_zp);
			break;
		case TMPP_UINT2INT:
			for (int i = 0; i < in_size; i++) out->data[i] = ((mtype_t) (((uint8_t *) (in->data))[i] - 128)) << UINT2INT_SHIFT;
			break;
#else
		case TMPP_UINT2FP01:
			for (int i = 0; i < in_size; i++) out->data[i] = (((uint8_t *) (in->data))[i]) / 255.0;
			break;
		case TMPP_UINT2FPN11:
			for (int i = 0; i < in_size; i++) out->data[i] = ((((uint8_t *) (in->data))[i]) - 128) / 128.0;
			break;
#endif
	...
	for (mdl->layer_i = 0; mdl->layer_i < mdl->b->layer_cnt; mdl->layer_i++) {
		...
		switch (h->type) {
			...
			case TML_DWCONV2D: {
				...
				res = tml_conv2d_dwconv2d(&_in, &_out, (wtype_t *) (mdl->layer_body + l->w_oft), (btype_t *) (mdl->layer_body + l->b_oft), l->kernel_w, l->kernel_h, l->stride_w,
										  l->stride_h, l->dilation_w, l->dilation_h, l->act, l->pad[0], l->pad[1], l->pad[2], l->pad[3], l->depth_mul,
			...
			case TML_FC: {
				...
				res = tml_fc(&_in, &_out, (wtype_t *) (mdl->layer_body + l->w_oft), (btype_t *) (mdl->layer_body + l->b_oft), (sctype_t *) (mdl->layer_body + l->ws_oft), h->in_s,
