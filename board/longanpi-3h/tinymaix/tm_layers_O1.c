...
//1x1 pw conv
TM_INLINE tm_err_t l_tml_pwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy, int dx, int dy, int act, int pad_top, int pad_bottom,
...
//normal conv
TM_INLINE tm_err_t l_tml_conv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy, int dx, int dy, int act, int pad_top, int pad_bottom,
								int pad_left, int pad_right, int dmul, sctype_t *ws, sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp) {
	int pad_flag = (pad_top != 0 || pad_bottom != 0 || pad_left != 0 || pad_right != 0);
	int maxk = kw * kh;
	int chi = in->c;
...
#error "unsupport mdl type"
			...
			wtype_t *kptr = (wtype_t *) w;
			for (; c < out->c - BATCH_SIZE + 1;) {
				for (int bat = 0; bat < BATCH_SIZE; bat += 2) tm_dot_prod_pack2(sptr, kptr + chi * maxk * bat, maxk * chi, sums + bat);
				tm_postprocess_sum(BATCH_SIZE, sums, b + c, act, outp, SUMSCALE, OUTSCALE, out_zp);
				c += BATCH_SIZE;
				outp += BATCH_SIZE;
...
//dwconv
TM_INLINE tm_err_t l_tml_dwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy, int dx, int dy, int act, int pad_top, int pad_bottom,
								  int pad_left, int pad_right, int dmul, sctype_t *ws, sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp) {
	int pad_flag = (pad_top != 0 || pad_bottom != 0 || pad_left != 0 || pad_right != 0);
	int maxk = kw * kh;
	int chi = in->c;
...
//dwconv
TM_INLINE tm_err_t l_tml_dwconv2d_3x3_part(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy, int dx, int dy, int act, int pad_top,
										   int pad_bottom, int pad_left, int pad_right, int dmul, sctype_t *ws, sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp,
...
// 40ms->27ms
TM_INLINE tm_err_t l_tml_dwconv2d_3x3_nostride(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy, int dx, int dy, int act, int pad_top,
											   int pad_bottom, int pad_left, int pad_right, int dmul, sctype_t *ws, sctype_t in_s, zptype_t in_zp, sctype_t out_s,
											   zptype_t out_zp) {
	int pad_flag = (pad_top != 0 || pad_bottom != 0 || pad_left != 0 || pad_right != 0);
	int maxk = kw * kh;
	int chi = in->c;
	...
	for (int y = 0; y + CONV_BLK_STEPY <= out->h; y += CONV_BLK_STEPY) {
		...
		for (int x = 0; x + CONV_BLK_STEPX <= out->w; x += CONV_BLK_STEPX) {
			...
			if (!slow_flag) {													//valid or same valid part
				...
				for (int c = 0; c < cho; c++) {
					wtype_t *kptr = (wtype_t *) w + c * maxk;
					sum0 = sptr[dw_koft[0]] * kptr[0] + sptr[dw_koft[1]] * kptr[1] + sptr[dw_koft[2]] * kptr[2] + sptr[dw_koft[4]] * kptr[3] + sptr[dw_koft[5]] * kptr[4] +
						   sptr[dw_koft[6]] * kptr[5] + sptr[dw_koft[8]] * kptr[6] + sptr[dw_koft[9]] * kptr[7] + sptr[dw_koft[10]] * kptr[8];
					sum1 = sptr[dw_koft[1]] * kptr[0] + sptr[dw_koft[2]] * kptr[1] + sptr[dw_koft[3]] * kptr[2] + sptr[dw_koft[5]] * kptr[3] + sptr[dw_koft[6]] * kptr[4] +
						   sptr[dw_koft[7]] * kptr[5] + sptr[dw_koft[9]] * kptr[6] + sptr[dw_koft[10]] * kptr[7] + sptr[dw_koft[11]] * kptr[8];
					sum2 = sptr[dw_koft[4]] * kptr[0] + sptr[dw_koft[5]] * kptr[1] + sptr[dw_koft[6]] * kptr[2] + sptr[dw_koft[8]] * kptr[3] + sptr[dw_koft[9]] * kptr[4] +
						   sptr[dw_koft[10]] * kptr[5] + sptr[dw_koft[12]] * kptr[6] + sptr[dw_koft[13]] * kptr[7] + sptr[dw_koft[14]] * kptr[8];
					sum3 = sptr[dw_koft[5]] * kptr[0] + sptr[dw_koft[6]] * kptr[1] + sptr[dw_koft[7]] * kptr[2] + sptr[dw_koft[9]] * kptr[3] + sptr[dw_koft[10]] * kptr[4] +
						   sptr[dw_koft[11]] * kptr[5] + sptr[dw_koft[13]] * kptr[6] + sptr[dw_koft[14]] * kptr[7] + sptr[dw_koft[15]] * kptr[8];
...
#error "unsupport mdl type"
				...
				for (int c = 0; c < out->c; c++) {
					...
					sum0 = sptr[0] * kptr[0] + sptr[1] * kptr[1] + sptr[2] * kptr[2] + sptr[4] * kptr[3] + sptr[5] * kptr[4] + sptr[6] * kptr[5] + sptr[8] * kptr[6] +
						   sptr[9] * kptr[7] + sptr[10] * kptr[8];
					sum1 = sptr[1] * kptr[0] + sptr[2] * kptr[1] + sptr[3] * kptr[2] + sptr[5] * kptr[3] + sptr[6] * kptr[4] + sptr[7] * kptr[5] + sptr[9] * kptr[6] +
						   sptr[10] * kptr[7] + sptr[11] * kptr[8];
					sum2 = sptr[4] * kptr[0] + sptr[5] * kptr[1] + sptr[6] * kptr[2] + sptr[8] * kptr[3] + sptr[9] * kptr[4] + sptr[10] * kptr[5] + sptr[12] * kptr[6] +
						   sptr[13] * kptr[7] + sptr[14] * kptr[8];
					sum3 = sptr[5] * kptr[0] + sptr[6] * kptr[1] + sptr[7] * kptr[2] + sptr[9] * kptr[3] + sptr[10] * kptr[4] + sptr[11] * kptr[5] + sptr[13] * kptr[6] +
						   sptr[14] * kptr[7] + sptr[15] * kptr[8];
					tm_postprocess_sum(1, &sum0, b + c, act, outp + 0 * cho, SUMSCALE, OUTSCALE, out_zp);
					tm_postprocess_sum(1, &sum1, b + c, act, outp + 1 * cho, SUMSCALE, OUTSCALE, out_zp);
					tm_postprocess_sum(1, &sum2, b + c, act, outp + (out->w + 0) * cho, SUMSCALE, OUTSCALE, out_zp);
					tm_postprocess_sum(1, &sum3, b + c, act, outp + (out->w + 1) * cho, SUMSCALE, OUTSCALE, out_zp);
	...
	if (out->w % CONV_BLK_W != 0 || out->h % CONV_BLK_H != 0) {//cal rest part
		...
		l_tml_dwconv2d_3x3_part(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp,
								out->w - out->w % CONV_BLK_W, out->w, 0, out->h / CONV_BLK_H * CONV_BLK_H);
		l_tml_dwconv2d_3x3_part(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp, 0, out->w,
...
TM_INLINE tm_err_t l_tml_mdwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy, int dx, int dy, int act, int pad_top, int pad_bottom,
								   int pad_left, int pad_right, int dmul, sctype_t *ws, sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp) {
	int pad_flag = (pad_top != 0 || pad_bottom != 0 || pad_left != 0 || pad_right != 0);
	int maxk = kw * kh;
	int chi = in->c;
							 ...
							 int pad_left, int pad_right, int dmul, sctype_t *ws, sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp)//kernel: (cho, chi, h, w)
{
	...
	if (maxk == 1) {
		...
		l_tml_pwconv2d(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp);
		TM_PERF_ADD(t_pwconv);
	} else if (dmul == 0) {
		TM_PERF_START(t_conv);//normal conv
		l_tml_conv2d(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp);
		TM_PERF_ADD(t_conv);
	} else if (dmul == 1) {
		TM_PERF_START(t_dwconv); //dw conv
		if (kh == 3 && kw == 3) {//opt for 3x3 dwconv
			if (sx == 1 && sy == 1) {
				l_tml_dwconv2d_3x3_nostride(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp);
			} else {
				l_tml_dwconv2d_3x3_part(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp, 0, out->w, 0,
										out->h);
			}
		} else {
			l_tml_dwconv2d(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp);
		}
		TM_PERF_ADD(t_dwconv);
	} else {//dmul>1 dwconv, rare
		l_tml_mdwconv2d(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp);
	}
