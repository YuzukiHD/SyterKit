...
static void set_pll_e90x(sunxi_ccu_t *ccu) {
	clrsetbits_le32(ccu->base[1] + E907_CLK_REG,
...
static void set_pll_a27l2(sunxi_ccu_t *ccu) {
	clrsetbits_le32(ccu->base[1] + A27L2_CLK_REG,
...
static void set_pll_peri_ctrl0(sunxi_ccu_t *ccu, uint32_t en,
	...
	clrsetbits_le32(ccu->base[1] + PLL_PERI_CTRL0_REG,
	...
	while (!(readl(ccu->base[1] + PLL_PERI_CTRL0_REG) &
...
static void set_pll_csi(sunxi_ccu_t *ccu) {
	...
	clrsetbits_le32(ccu->base[1] + PLL_CSI_CTRL_REG,
	...
	/* Wait Lock_status */
	while (!(readl(ccu->base[1] + PLL_CSI_CTRL_REG) &
...
static void set_pll_video(sunxi_ccu_t *ccu) {
	...
	clrsetbits_le32(ccu->base[1] + PLL_VIDEO_CTRL_REG,
	...
	/* Wait Lock_status */
	while (!(readl(ccu->base[1] + PLL_VIDEO_CTRL_REG) &
...
static void set_ahb(sunxi_ccu_t *ccu) {
	clrsetbits_le32(ccu->base[1] + AHB_CLK_REG,
...
static void set_apb(sunxi_ccu_t *ccu) {
	clrsetbits_le32(ccu->base[1] + APB_CLK_REG,
...
static void set_apb_spec(sunxi_ccu_t *ccu) {
	clrsetbits_le32(ccu->base[1] + APB_SPEC_CLK_REG,
...
static void sun300iw1_clk_pre_init(sunxi_ccu_t *ccu) {
	set_apb_spec(ccu);
	set_pll_peri(ccu);
}
...
void sunxi_clk_dump(sunxi_ccu_t *ccu) {
	...
	reg_val = readl(ccu->base[1] + E907_CLK_REG);
	...
	switch (clock_src) {
		...
		case E907_CLK_REG_E907_CLK_SEL_VIDEOPLL2X:
			reg_val = readl(ccu->base[1] + PLL_VIDEO_CTRL_REG);
		...
		case E907_CLK_REG_E907_CLK_SEL_CPU_PLL:
			pll_cpu_reg = readl(ccu->base[1] + PLL_CPU_CTRL_REG);
	...
	reg_val = read32(ccu->base[1] + PLL_PERI_CTRL0_REG);
