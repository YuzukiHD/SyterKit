...
static void set_pll_e90x(sunxi_ccu_t *ccu) {
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, E907_CLK_REG),
...
static void set_pll_a27l2(sunxi_ccu_t *ccu) {
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, A27L2_CLK_REG),
...
static void set_pll_peri_ctrl0(sunxi_ccu_t *ccu, uint32_t en,
	...
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, PLL_PERI_CTRL0_REG),
	...
	while (!(readl(sunxi_ccu_aon_reg(ccu, PLL_PERI_CTRL0_REG)) &
...
static void set_pll_peri_ctrl1(sunxi_ccu_t *ccu) {
	uint32_t reg;
	reg = readl(sunxi_ccu_aon_reg(ccu, PLL_PERI_CTRL1_REG));
	// reg |= PLL_PERI_CTRL1_REG_PLL_PERI_CKO_192_EN_ENABLE
	//        << PLL_PERI_CTRL1_REG_PLL_PERI_CKO_192_EN_OFFSET;
	reg = 0xFFFF;
	writel(reg, sunxi_ccu_aon_reg(ccu, PLL_PERI_CTRL1_REG));
	return;
}

/* pll peri hosc*2N/M = 3072M  hardware * 2 */
static void set_pll_peri(sunxi_ccu_t *ccu) {
	if (!(readl(sunxi_ccu_aon_reg(ccu, PLL_PERI_CTRL0_REG)) &
	      PLL_PERI_CTRL0_REG_PLL_EN_CLEAR_MASK)) {
		if (sunxi_clk_get_hosc_type(ccu) == HOSC_FREQ_40M) {
			set_pll_peri_ctrl0(ccu,
	...
	set_pll_peri_ctrl1(ccu);
	return;
}

/* pll csi rate = 675M = hosc / 4 * N , N = N(INT) + N(FRAC) */
static void set_pll_csi(sunxi_ccu_t *ccu) {
	...
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, PLL_CSI_CTRL_REG),
	...
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, PLL_CSI_PAT0_CTRL_REG),
			    PLL_CSI_PAT0_CTRL_REG_WAVE_BOT_CLEAR_MASK,
			    wave_bot << PLL_CSI_PAT0_CTRL_REG_WAVE_BOT_OFFSET);

	/* Enable SIG_DELT_PAT_EN of PAT1 */
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, PLL_CSI_PAT1_CTRL_REG),
	...
	/* Wait Lock_status */
	while (!(readl(sunxi_ccu_aon_reg(ccu, PLL_CSI_CTRL_REG)) &
...
static void set_pll_video(sunxi_ccu_t *ccu) {
	/* Close Lock_en and Output_gate */
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, PLL_VIDEO_CTRL_REG),
	...
	/* Wait Lock_status */
	while (!(readl(sunxi_ccu_aon_reg(ccu, PLL_VIDEO_CTRL_REG)) &
...
static void set_ahb(sunxi_ccu_t *ccu) {
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, AHB_CLK_REG),
...
static void set_apb(sunxi_ccu_t *ccu) {
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, APB_CLK_REG),
...
// 192M
static void set_apb_spec(sunxi_ccu_t *ccu) {
	clrsetbits_le32(sunxi_ccu_aon_reg(ccu, APB_SPEC_CLK_REG),
...
void sunxi_clk_dump(sunxi_ccu_t *ccu) {
	...
	printk_debug("SoC HOSC Type = %d MHz\n", sunxi_clk_get_hosc_type(ccu));

	reg_val = readl(sunxi_ccu_aon_reg(ccu, E907_CLK_REG));
	...
	switch (clock_src) {
		case E907_CLK_REG_E907_CLK_SEL_HOSC:
			clock = sunxi_clk_get_hosc_type(ccu);
			break;
		case E907_CLK_REG_E907_CLK_SEL_VIDEOPLL2X:
			reg_val = readl(sunxi_ccu_aon_reg(
			...
			clock = (sunxi_clk_get_hosc_type(ccu) * 2 * (n + 1)) / (m + 1);
			break;
		...
		case E907_CLK_REG_E907_CLK_SEL_CPU_PLL:
			pll_cpu_reg = readl(sunxi_ccu_aon_reg(
					ccu, PLL_CPU_CTRL_REG));
			n = (pll_cpu_reg & PLL_CPU_CTRL_REG_PLL_N_CLEAR_MASK) >> PLL_CPU_CTRL_REG_PLL_N_OFFSET;
			clock = sunxi_clk_get_hosc_type(ccu) * (n + 1);
			break;
	...
	reg_val = read32(sunxi_ccu_aon_reg(ccu, PLL_PERI_CTRL0_REG));
	...
	printk_debug("CLK: PERI FREQ = %lu MHz\r\n", (sunxi_clk_get_hosc_type(ccu) * 2 * (n + 1)) / (m + 1));
}
...
static int sunxi_clk_pre_initcall(void) {
	...
	result = sunxi_ccu_dt_read(&ccu);
	if (result != DRIVER_OK)
		return result;
	sunxi_clk_pre_init(&ccu);
	return DRIVER_OK;
}

early_initcall(sunxi_clk_pre_initcall);

DT2C_DRIVER_COMPAT("allwinner,sun300iw1-ccu");
