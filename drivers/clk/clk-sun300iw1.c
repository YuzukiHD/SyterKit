 ...
 */
static void set_pll_general(
	uintptr_t pll_addr, uint32_t en, uint32_t output_gate_en, uint32_t pll_d, uint32_t pll_d_off, uint32_t pll_n)
{
	uint32_t pll_en;
	uint32_t pll_ldo_en;
...
{
	clrsetbits_le32(SUNXI_CCU_AON_BASE + E907_CLK_REG, E907_CLK_REG_E907_CLK_DIV_CLEAR_MASK,
		CCU_E90X_CLK_CPU_M_1 << E907_CLK_REG_E907_CLK_DIV_OFFSET);
	clrsetbits_le32(SUNXI_CCU_AON_BASE + E907_CLK_REG, E907_CLK_REG_E907_CLK_SEL_CLEAR_MASK,
		E907_CLK_REG_E907_CLK_SEL_PERI_PLL_614M << E907_CLK_REG_E907_CLK_SEL_OFFSET);
	return;
}
...
{
	clrsetbits_le32(SUNXI_CCU_AON_BASE + A27L2_CLK_REG, A27L2_CLK_REG_A27L2_CLK_DIV_CLEAR_MASK,
		CCU_A27_CLK_CPU_M_1 << A27L2_CLK_REG_A27L2_CLK_DIV_OFFSET);
	clrsetbits_le32(SUNXI_CCU_AON_BASE + A27L2_CLK_REG, A27L2_CLK_REG_A27L2_CLK_SEL_CLEAR_MASK,
		A27L2_CLK_REG_A27L2_CLK_SEL_CPU_PLL << A27L2_CLK_REG_A27L2_CLK_SEL_OFFSET);
	clrsetbits_le32(SUNXI_CCU_AON_BASE + A27L2_CLK_REG, A27L2_CLK_REG_A27L2_CLK_EN_CLEAR_MASK,
		A27L2_CLK_REG_A27L2_CLK_EN_CLOCK_IS_ON << A27L2_CLK_REG_A27L2_CLK_EN_OFFSET);
	return;
}
...
static void set_pll_peri_ctrl0(uint32_t en, uint32_t output_gate_en, uint32_t pll_n, uint32_t pll_m)
{
	...
	clrsetbits_le32(
		SUNXI_CCU_AON_BASE + PLL_PERI_CTRL0_REG, PLL_PERI_CTRL0_REG_LOCK_ENABLE_CLEAR_MASK, pll_lock_en);

	while (!(readl(SUNXI_CCU_AON_BASE + PLL_PERI_CTRL0_REG) & PLL_PERI_CTRL0_REG_LOCK_CLEAR_MASK))
		;

	if (output_gate_en == 1) {
		pll_output_gate = PLL_PERI_CTRL0_REG_PLL_OUTPUT_GATE_ENABLE
				  << PLL_PERI_CTRL0_REG_PLL_OUTPUT_GATE_OFFSET;
		clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_PERI_CTRL0_REG, PLL_PERI_CTRL0_REG_PLL_OUTPUT_GATE_CLEAR_MASK,
			pll_output_gate);
	}
}
...
static void set_pll_peri(void)
{
	if (!(readl(SUNXI_CCU_AON_BASE + PLL_PERI_CTRL0_REG) & PLL_PERI_CTRL0_REG_PLL_EN_CLEAR_MASK)) {
		if (sun300iw1_clk_get_hosc_rate() == HOSC_FREQ_40M) {
			set_pll_peri_ctrl0(PLL_PERI_CTRL0_REG_PLL_EN_ENABLE, PLL_PERI_CTRL0_REG_PLL_OUTPUT_GATE_ENABLE,
				CCU_AON_PLL_CPU_N_192, CCU_AON_PLL_CPU_M_5);
		} else {
			set_pll_peri_ctrl0(PLL_PERI_CTRL0_REG_PLL_EN_ENABLE, PLL_PERI_CTRL0_REG_PLL_OUTPUT_GATE_ENABLE,
				CCU_AON_PLL_CPU_N_192, CCU_AON_PLL_CPU_M_3);
		}
	}
	set_pll_peri_ctrl1();
	return;
...
static void set_pll_csi(void)
{
	...
	clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_CSI_CTRL_REG, PLL_CSI_CTRL_REG_PLL_FACTOR_N_CLEAR_MASK,
		n << PLL_CSI_CTRL_REG_PLL_FACTOR_N_OFFSET);
	clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_CSI_CTRL_REG, PLL_CSI_CTRL_REG_PLL_INPUT_DIV_CLEAR_MASK,
		input_div << PLL_CSI_CTRL_REG_PLL_INPUT_DIV_OFFSET);

	/* Enable PLL_EN PLL_LEO_EN */
	clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_CSI_CTRL_REG, PLL_CSI_CTRL_REG_PLL_EN_CLEAR_MASK,
		PLL_CSI_CTRL_REG_PLL_EN_ENABLE << PLL_CSI_CTRL_REG_PLL_EN_OFFSET);
	clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_PERI_CTRL0_REG, PLL_CSI_CTRL_REG_PLL_LDO_EN_CLEAR_MASK,
		PLL_CSI_CTRL_REG_PLL_LDO_EN_ENABLE << PLL_CSI_CTRL_REG_PLL_LDO_EN_OFFSET);

	/* Enable SDM */
	clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_CSI_CTRL_REG, PLL_CSI_CTRL_REG_PLL_SDM_EN_CLEAR_MASK,
		PLL_CSI_CTRL_REG_PLL_SDM_EN_ENABLE << PLL_CSI_CTRL_REG_PLL_SDM_EN_OFFSET);

	/* Set WAVE_BOT and SPR_FREQ_MODE of PAT0 */
	clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_CSI_PAT0_CTRL_REG, PLL_CSI_PAT0_CTRL_REG_WAVE_BOT_CLEAR_MASK,
		wave_bot << PLL_CSI_PAT0_CTRL_REG_WAVE_BOT_OFFSET);

	/* Enable SIG_DELT_PAT_EN of PAT1 */
	clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_CSI_PAT1_CTRL_REG, PLL_CSI_PAT1_CTRL_REG_SIG_DELT_PAT_EN_CLEAR_MASK,
		0x1 << PLL_CSI_PAT1_CTRL_REG_SIG_DELT_PAT_EN_OFFSET);
	...
	clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_CSI_CTRL_REG, PLL_CSI_CTRL_REG_LOCK_ENABLE_CLEAR_MASK,
		PLL_CSI_CTRL_REG_LOCK_ENABLE_ENABLE << PLL_CSI_CTRL_REG_LOCK_ENABLE_OFFSET);
...
static void set_pll_video(void)
{
	...
	/* Set N、M */
	if (sun300iw1_clk_get_hosc_rate() == HOSC_FREQ_40M) {
		clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_VIDEO_CTRL_REG, PLL_VIDEO_CTRL_REG_PLL_N_CLEAR_MASK,
			CCU_AON_PLL_CPU_N_30 << PLL_VIDEO_CTRL_REG_PLL_N_OFFSET);
	} else {
		clrsetbits_le32(SUNXI_CCU_AON_BASE + PLL_VIDEO_CTRL_REG, PLL_VIDEO_CTRL_REG_PLL_N_CLEAR_MASK,
			CCU_AON_PLL_CPU_N_50 << PLL_VIDEO_CTRL_REG_PLL_N_OFFSET);
	}
...
{
	clrsetbits_le32(SUNXI_CCU_AON_BASE + AHB_CLK_REG, AHB_CLK_REG_AHB_CLK_DIV_CLEAR_MASK,
		CCU_AON_PLL_CPU_M_4 << AHB_CLK_REG_AHB_CLK_DIV_OFFSET);
	udelay(2);
	clrsetbits_le32(SUNXI_CCU_AON_BASE + AHB_CLK_REG, AHB_CLK_REG_AHB_SEL_CLEAR_MASK,
		AHB_CLK_REG_AHB_SEL_PERI_768M << AHB_CLK_REG_AHB_SEL_OFFSET);
	udelay(2);
	return;
}
...
{
	clrsetbits_le32(SUNXI_CCU_AON_BASE + APB_CLK_REG, APB_CLK_REG_APB_CLK_DIV_CLEAR_MASK,
		CCU_AON_PLL_CPU_M_4 << APB_CLK_REG_APB_CLK_DIV_OFFSET);
	udelay(2);
	clrsetbits_le32(SUNXI_CCU_AON_BASE + APB_CLK_REG, APB_CLK_REG_APB_SEL_CLEAR_MASK,
		APB_CLK_REG_APB_SEL_PERI_384M << APB_CLK_REG_APB_SEL_OFFSET);
	udelay(2);
	return;
}
...
static void set_apb_spec(void)
{
	clrsetbits_le32(SUNXI_CCU_AON_BASE + APB_SPEC_CLK_REG,
		APB_SPEC_CLK_REG_APB_SPEC_SEL_CLEAR_MASK | APB_SPEC_CLK_REG_APB_SPEC_CLK_DIV_CLEAR_MASK,
...
void sunxi_clk_init(void)
{
	/* detect hosc */
	if (sun300iw1_clk_get_hosc_rate() == HOSC_FREQ_40M) {
		set_pll_general(SUNXI_CCU_AON_BASE + PLL_CPU_CTRL_REG, PLL_CPU_CTRL_REG_PLL_EN_ENABLE,
			PLL_CPU_CTRL_REG_PLL_OUTPUT_GATE_ENABLE, CCU_AON_PLL_CPU_D_1, 2, CCU_AON_PLL_CPU_N_27);
		if (!(readl(SUNXI_CCU_AON_BASE + PLL_VIDEO_CTRL_REG) & PLL_CPU_CTRL_REG_PLL_EN_CLEAR_MASK)) {
			set_pll_general(SUNXI_CCU_AON_BASE + PLL_VIDEO_CTRL_REG, PLL_CPU_CTRL_REG_PLL_EN_ENABLE,
				PLL_CPU_CTRL_REG_PLL_OUTPUT_GATE_ENABLE, CCU_AON_PLL_CPU_D_4, 1, CCU_AON_PLL_CPU_N_118);
		}
	} else {
		set_pll_general(SUNXI_CCU_AON_BASE + PLL_CPU_CTRL_REG, PLL_CPU_CTRL_REG_PLL_EN_ENABLE,
			PLL_CPU_CTRL_REG_PLL_OUTPUT_GATE_ENABLE, CCU_AON_PLL_CPU_D_1, 2, CCU_AON_PLL_CPU_N_45);
		if (!(readl(SUNXI_CCU_AON_BASE + PLL_VIDEO_CTRL_REG) & PLL_CPU_CTRL_REG_PLL_EN_CLEAR_MASK)) {
			set_pll_general(SUNXI_CCU_AON_BASE + PLL_VIDEO_CTRL_REG, PLL_CPU_CTRL_REG_PLL_EN_ENABLE,
				PLL_CPU_CTRL_REG_PLL_OUTPUT_GATE_ENABLE, CCU_AON_PLL_CPU_D_2, 1, CCU_AON_PLL_CPU_N_99);
		}
	}
