...
#define pr_fmt(fmt) "clk-sun50iw9: " fmt
...
static inline void set_pll_cpux_axi(void)
{
	...
	/* disable pll */
	reg_val = read32(SUNXI_CCM_BASE + CCU_PLL_CPUX_CTRL_REG) & ~(1 << 31);
	...
	/* lock enable */
	reg_val = read32(SUNXI_CCM_BASE + CCU_PLL_CPUX_CTRL_REG) | (1 << 29);
	writel(reg_val, SUNXI_CCM_BASE + CCU_PLL_CPUX_CTRL_REG);
	...
	while (!(read32(SUNXI_CCM_BASE + CCU_PLL_CPUX_CTRL_REG) & (0x1 << 28)))
		;
	sdelay(20);
	...
	reg_val = (read32(SUNXI_CCM_BASE + CCU_CPUX_AXI_CFG_REG) & ~(0x03 << 24)) | (0x03 << 24);
...
static inline void set_pll_periph0(void)
{
	...
	if ((1U << 31) & read32(SUNXI_CCM_BASE + CCU_PLL_PERI0_CTRL_REG)) {
	...
	/* lock enable */
	reg_val = read32(SUNXI_CCM_BASE + CCU_PLL_PERI0_CTRL_REG);
	reg_val |= (1 << 29);
...
static inline void set_apb(void)
{
	...
	writel((0x03 << 24) | read32(SUNXI_CCM_BASE + CCU_APB1_CFG_GREG), SUNXI_CCM_BASE + CCU_APB1_CFG_GREG);
	sdelay(1);
}
...
{
	/*dma reset*/
	writel(read32(SUNXI_CCM_BASE + CCU_DMA_BGR_REG) | (1 << 16), SUNXI_CCM_BASE + CCU_DMA_BGR_REG);
	sdelay(20);
...
static inline void set_pll_mbus(void)
{
	...
	/* set MBUS div */
	reg_val = read32(SUNXI_CCM_BASE + CCU_MBUS_CFG_REG);
	reg_val |= (2 << 0);
...
static inline void set_circuits_analog(void)
{
	...
	reg_val = read32(SUNXI_RPRCM_BASE + 0x250U);
	reg_val |= (0x01 << VDD_ADDA_OFF_GATING);
	writel(reg_val, SUNXI_RPRCM_BASE + 0x250U);
	sdelay(1);

	reg_val = read32(SUNXI_RPRCM_BASE + 0x310U);
	reg_val |= (0x01 << CAL_ANA_EN);
	writel(reg_val, SUNXI_RPRCM_BASE + 0x310U);
	sdelay(1);
...
static inline void set_modules_clock(void)
{
	...
	const uintptr_t modules_reg_addrs[] = {
		SUNXI_CCM_BASE + 0x28U, // peri1 clk
		SUNXI_CCM_BASE + 0x30U, // gpu clk
		SUNXI_CCM_BASE + 0x40U, // video0 clk
		SUNXI_CCM_BASE + 0x48U, // video1 clk
		SUNXI_CCM_BASE + 0x50U, // video2 clk
		SUNXI_CCM_BASE + 0x58U, // ve clk
		SUNXI_CCM_BASE + 0x60U, // de clk
		SUNXI_CCM_BASE + 0xe0U, // csi clk
		SUNXI_CCM_BASE + 0x78U // audio clk
	};
...
static inline int sunxi_clock_init_gpadc(void)
{
	uint32_t reg_val = 0;
	/* reset */
	reg_val = read32(SUNXI_CCM_BASE + CCU_GPADC_BGR_REG);
	reg_val &= ~(1 << 16);
	writel(reg_val, SUNXI_CCM_BASE + CCU_GPADC_BGR_REG);
...
{
	pr_debug("Set SoC 1823 (H616/H313/H618) CLK Start.\n");
	set_platform_config();
	set_pll_cpux_axi();
	set_pll_periph0();
	set_ahb();
