...
#define pr_fmt(fmt) "clk-sun8iw22: " fmt
...
void sunxi_clk_init(void)
{
	...
	/* Update PLL */
	setbits_le32(SUNXI_CCMU_BASE + PLL_CPU_CTRL_REG, (0x1U << CPU_UPDATE_OFFSET));
	/* Wait for hardware to clear update bit */
	while (readl(SUNXI_CCMU_BASE + PLL_CPU_CTRL_REG) & (0x1U << CPU_UPDATE_OFFSET))
		;

	/* Lock enable */
	clrbits_le32(SUNXI_CCMU_BASE + PLL_CPU_CTRL_REG, (0x1U << CPU_LOCK_ENABLE_OFFSET));
	udelay(10);
	setbits_le32(SUNXI_CCMU_BASE + PLL_CPU_CTRL_REG, (0x1U << CPU_LOCK_ENABLE_OFFSET));

	/* Wait for lock */
	judge_cnt = 0;
	do {
		udelay(3);
		reg_val = readl(SUNXI_CCMU_BASE + PLL_CPU_CTRL_REG);
	...
	clrbits_le32(SUNXI_CPUX_PLL_CFG_BASE + 0x20U, (0x3 << 16));

	/* Set CPU-AXI divider factor M */
	clrsetbits_le32(SUNXI_CPUX_PLL_CFG_BASE + 0x20U, (0x3 << 0), (0x1 << 0));
	udelay(10);

	/* Set CPU clock source */
	clrsetbits_le32(SUNXI_CPUX_PLL_CFG_BASE + 0x18U, (0x7 << 24), (0x3 << 24));
	udelay(10);
}
