...
void sunxi_clk_init(void) {
	...
	/* 24M*n/p/(m0 * m1) */
	clrsetbits_le32(SUNXI_CCMU_BASE + PLL_CPU_CTRL_REG,
			(0xffU << 8) | (0x7U << 16) | (0x3U << 20) | (0xfU << 0),
			(0x2a << 8) | (0x0 << 0));
	udelay(10);

	/* Update PLL */
	setbits_le32(SUNXI_CCMU_BASE + PLL_CPU_CTRL_REG,
		     (0x1U << CPU_UPDATE_OFFSET));
	/* Wait for hardware to clear update bit */
	while (readl(SUNXI_CCMU_BASE + PLL_CPU_CTRL_REG) &
	       (0x1U << CPU_UPDATE_OFFSET))
		;

	/* Lock enable */
	clrbits_le32(SUNXI_CCMU_BASE + PLL_CPU_CTRL_REG,
	...
	clrsetbits_le32(SUNXI_CPUX_PLL_CFG_BASE + 0x20U,
			(0x3 << 0), (0x1 << 0));
	udelay(10);

	/* Set CPU clock source */
	clrsetbits_le32(SUNXI_CPUX_PLL_CFG_BASE + 0x18U,
			(0x7 << 24), (0x3 << 24));
	udelay(10);
}
