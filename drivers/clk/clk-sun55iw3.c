...
static void enable_pll(uintptr_t cpux, core_pll_freq_fact *CPUx,
...
static void set_pll_cpux_axi(sunxi_ccu_t *ccu) {
	...
	writel(0x0305, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x60U));
	writel(0x0305, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x64U));
	sdelay(20);
	writel(0, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x6cU));
	sdelay(20);
	...
	cpu_pll.FactorM1 = 0;
	cpu_pll.FactorP = 0;
	enable_pll(sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x04U), &cpu_pll, 0x48801400);

	cpu_pll.FactorM0 = 0;
	cpu_pll.FactorN = 0x2a; /*1008M*/
	cpu_pll.FactorM1 = 0;
	cpu_pll.FactorP = 0;
	enable_pll(sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x0cU), &cpu_pll, 0x48801400);

	cpu_pll.FactorM0 = 0;
	cpu_pll.FactorN = 0x1d; /*696M*/
	cpu_pll.FactorM1 = 0;
	cpu_pll.FactorP = 0;
	enable_pll(sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x08U), &cpu_pll, 0x48801400);

	/* PLL_CPU1 is core0~core3 clock,  select PLL_CPU1  clock src:
	 * PLL_CPU1/P ,P = 1,  */
	/*set and change cpu clk src to PLL_CPU1,  PLL_CPU1/P*/
	reg_val = readl(sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x60U));
	reg_val &= ~(0x03 << 24);
	reg_val |= (0x03 << 24);
	reg_val &= ~(0x01 << 16);// P = 1
	reg_val |= (0x00 << 16);
	writel(reg_val, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x60U));
	sdelay(20);

	/* PLL_CPU3 is core4~core7 clock,  select PLL_CPU3  clock src:
	 * PLL_CPU3/P ,P = 1,  */
	/*set and change cpu clk src to PLL_CPU1,  PLL_CPU1/P*/
	reg_val = readl(sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x64U));
	reg_val &= ~(0x03 << 24);
	reg_val |= (0x03 << 24);
	reg_val &= ~(0x01 << 16);// P = 1
	reg_val |= (0x00 << 16);
	writel(reg_val, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x64U));
	sdelay(20);

	/*dsu clk src to PLL_CPU2,  PLL_CPU2/P*/
	reg_val = readl(sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x6cU));
	reg_val &= ~(0x03 << 24);
	reg_val |= (0x03 << 24);
	reg_val &= ~(0x01 << 16);// P = 1
	reg_val |= (0x00 << 16);
	writel(reg_val, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x6cU));
	sdelay(20);
}

static void set_pll_periph0(sunxi_ccu_t *ccu) {
	uint32_t reg_val;
	...
	writel((0x48216310), sunxi_ccu_reg(ccu, CCU_PLL_PERI0_CTRL_REG));
...
static void set_pll_periph1(sunxi_ccu_t *ccu) {
	uint32_t reg_val;

	if ((1U << 31) & readl(sunxi_ccu_reg(ccu, CCU_PLL_PERI1_CTRL_REG))) {
		/*fel has enable pll_periph0*/
		printk_debug("periph0 has been enabled\n");
		return;
	}
	...
	/* set default val  24MHz * N/M1/P0 = 24 * 100 /1/2 = 1.2G*/
	writel((0x48216310), sunxi_ccu_reg(ccu, CCU_PLL_PERI1_CTRL_REG));

	/* lock enable */
	reg_val = readl(sunxi_ccu_reg(ccu, CCU_PLL_PERI1_CTRL_REG));
	reg_val |= (1 << 29);
	writel(reg_val, sunxi_ccu_reg(ccu, CCU_PLL_PERI1_CTRL_REG));
	...
	while (!(readl(sunxi_ccu_reg(ccu, CCU_PLL_PERI1_CTRL_REG)) & (0x1 << 28)))
		;
	sdelay(20);
	/* lock disable */
	reg_val = readl(sunxi_ccu_reg(ccu, CCU_PLL_PERI1_CTRL_REG));
...
static void set_ahb(sunxi_ccu_t *ccu) {
	/* PLL6:AHB1:APB1 = 600M:200M  AHB = src clk/M */
	writel((2 << 0), sunxi_ccu_reg(ccu, CCU_AHB0_CFG_REG));
	writel((0x03 << 24) | readl(sunxi_ccu_reg(ccu, CCU_AHB0_CFG_REG)), sunxi_ccu_reg(ccu, CCU_AHB0_CFG_REG));
	sdelay(1);
}

static void set_apb(sunxi_ccu_t *ccu) {
	/*PLL6:APB0 = 600M:100M  APB = src clk/M */
	writel((5 << 0), sunxi_ccu_reg(ccu, CCU_APB0_CFG_REG));
	writel((0x03 << 24) | readl(sunxi_ccu_reg(ccu, CCU_APB0_CFG_REG)), sunxi_ccu_reg(ccu, CCU_APB0_CFG_REG));
	sdelay(1);
}

static void set_pll_dma(sunxi_ccu_t *ccu) {
	/*dma reset*/
...
static void set_circuits_analog(sunxi_ccu_t *ccu) {
	/* calibration circuits analog enable */
	setbits_le32(sunxi_ccu_r_prcm_reg(ccu, 0x250U),
		     0x01 << VDD_ADDA_OFF_GATING);
	sdelay(1);

	setbits_le32(sunxi_ccu_r_prcm_reg(ccu, 0x310U),
		     0x01 << CAL_ANA_EN);
	sdelay(1);

	clrbits_le32(sunxi_ccu_r_prcm_reg(ccu, 0x310U),
		     0x01 << CAL_EN);
	sdelay(1);
...
static void set_platform_config(sunxi_ccu_t *ccu) {
	set_circuits_analog(ccu);
	set_iommu_auto_gating(ccu);
}

static void set_modules_clock(sunxi_ccu_t *ccu) {
	uint32_t reg_val, i;
...
void sunxi_clk_reset(sunxi_ccu_t *ccu) {
	...
	reg_val = readl(sunxi_ccu_reg(ccu, CCU_AHB0_CFG_REG));
	reg_val &= (~(0x3 << 24));
	writel(reg_val, sunxi_ccu_reg(ccu, CCU_AHB0_CFG_REG));

	reg_val = readl(sunxi_ccu_reg(ccu, CCU_APB0_CFG_REG));
	reg_val &= (~(0x3 << 24));
	writel(reg_val, sunxi_ccu_reg(ccu, CCU_APB0_CFG_REG));
...
static void sunxi_cpux_clk_dump(sunxi_ccu_t *ccu, uint8_t cpuid,
				uintptr_t cpu_reg) {
	uint32_t reg_val;
...
void sunxi_clk_dump(sunxi_ccu_t *ccu) {
	...
	sunxi_cpux_clk_dump(ccu, 0, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x00U));
	sunxi_cpux_clk_dump(ccu, 1, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x04U));
	sunxi_cpux_clk_dump(ccu, 2, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x08U));
	sunxi_cpux_clk_dump(ccu, 3, sunxi_ccu_cpu_sys_cfg_reg(ccu, 0x0cU));
...
DT2C_DRIVER_COMPAT("allwinner,sun55iw3-ccu");
