...
static int prepare_dma(sdhci_t *sdhci, sdhci_data_t *data) {
	volatile sdhci_idma_desc_t *pdes = sdhci->dma_desc;
	...
	for (i = 0; i < buff_frag_num; i++, des_idx++) {
		...
		pdes[des_idx].buf_addr = ((uint32_t) (uintptr_t) buff + i * SMHC_DES_BUFFER_MAX_LEN) >> 2;
		if (i == 0)
		...
		if (i == buff_frag_num - 1) {
			...
			pdes[des_idx].next_desc_addr = ((uint32_t) (uintptr_t) &pdes[des_idx + 1]) >> 2;
		}
	...
	sdhci->reg->dlba = (uint32_t) (uintptr_t) pdes >> 2;
