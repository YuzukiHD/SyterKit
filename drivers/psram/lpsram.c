...
#define pr_fmt(fmt) "lpsram: " fmt

/*
 * @file lpsram.c
 * @brief Reconstructed C source of the Allwinner F101 (sun252iw2) LPSRAM
 *        boot-init library (libpsram.a, "LPSRAM BOOT DRIVER V0.56").
 *
 * The original library is shipped as a prebuilt RV32 object; this file is a
 * reverse-engineered equivalent so the same initialisation sequence can be
 * built for RV64 (C907).  Register write sequences were transcribed from the
 * disassembly of the original object and are kept as close to the original as
 * practical, including its quirks.
 *
 * External dependencies (all provided by SyterKit):
 *   printf_dram(), dcache_enable(), dcache_disable(), flush_dcache_all(),
 *   memcpy(), memset()
 */
...
extern int printf_dram(const char *fmt, ...);
