...
static const char *get_dram_type_name(uint32_t type)
{
	size_t i;
...
static void sid_pwr_ctrl(void)
{
	writel(0x80000000, SID_PWR_BASE + 0x310);
	writel(1, SID_PWR_BASE + 0x204);
}

static uint32_t sid_read_key(uint32_t key)
{
	uint32_t val;
...
static uint32_t lpsram_get_type_by_hwid(void)
{
...
static int lpsram_para_merge_config(lpsram_para_t *para)
{
	uint32_t width;
	uint32_t type;
...
static uint32_t dram_vol_set(lpsram_para_t *para)
{
...
static void lpsram_enable_all_master(void)
{
...
static void lpsram_disable_all_master(void)
{
...
static void lpsram_reset_cfg(void)
{
...
static void lpsram_mr_write(lpsram_para_t *para, uint32_t mreg, uint32_t value, uint32_t hi)
{
	...
	case 3:
		...
		writel(((value << 8) & 0xffff) | ((mreg << 16) & 0xff0000), LPSRAM_MCTL_220c);
		break;
	case 4:
		writel(0xc0c00000 | ((mreg << 24) & 0x07000000) | ((mreg << 16) & 0x00070000) | (value & 0xff),
			LPSRAM_MCTL_2210);
		writel(0, LPSRAM_MCTL_220c);
		break;
	default: /* types 5..8 */
		writel(0x60000100, LPSRAM_MCTL_2210);
		writel(((hi << 8) & 0xffff) | (value & 0xff) | ((mreg << 16) & 0xff0000), LPSRAM_MCTL_220c);
		break;
	}
	lpsram_mr_trigger(0xd);
}

static uint32_t lpsram_mr_read(lpsram_para_t *para, uint32_t mreg)
{
	switch (para->dram_type) {
	case 1:
	...
	case 3:
		...
		break;
	case 4:
		writel(0x40400000 | ((mreg << 24) & 0x07000000) | ((mreg << 16) & 0x00070000), LPSRAM_MCTL_2210);
		writel(0, LPSRAM_MCTL_220c);
		break;
	default: /* types 5..8 */
...
static uint32_t ccm_set_pll_ldram_clk(uint32_t clk)
{
	if (clk > 200) {
		uint32_t n = clk / 24;
		uint32_t v;
...
static uint32_t lpsram_sys_init_cfg(lpsram_para_t *para)
{
...
static void lpsram_phy_dx_bit_delay_compensation(lpsram_para_t *para)
{
	...
	for (i = 0; i < 8; i++) {
		...
		v = (v & 0xffffc0c0) | ((uint32_t)dram_rdqbit[i] << 1 & 0x1e) |
	...
	case 5:
		v = readl(0x02053058);
		v = (v & 0x8000007f) | ((tpr11 >> 8) & 0xf00) | ((tpr10 << 12) & 0x00f00000) |
		    ((tpr11 >> 6) & 0x0003c000) | ((tpr10 << 14) & 0x3c000000);
		writel(v, 0x02053058);
		break;
	case 3:
	case 7:
	case 8:
		v = readl(0x02053058);
		v = (v & 0x8000007f) | ((tpr10 << 12) & 0x00f00000) | ((tpr11 >> 8) & 0xf00) |
		    ((tpr11 >> 20 << 8) & 0xf00);
		writel(v, 0x02053058);
		break;
	default:
...
static void lpsram_vrefzq_init(lpsram_para_t *para)
{
...
static void lpsram_db_remap_config(uint32_t mask, const uint32_t *src, const uint32_t *dst)
{
	int i;
	...
	writel((readl(LPSRAM_REMAP_BASE) & ~0x1eu) | ((mask << 1) & 0x1e), LPSRAM_REMAP_BASE);
}
...
static uint32_t msi_phy_initial_cfg(lpsram_para_t *para)
{
	...
	v = (v & 0xf8000000) | ((para->dram_zq & 0xffffff) ? (para->dram_zq & 0xffffff) : 0x3b3bbb);
	v |= 1u << 25;
...
static void lpsram_controller_initial_finish(lpsram_para_t *para)
{
	...
	writel((readl(LPSRAM_MCTL_2154) & 0xff000000) | val | 0x880005, LPSRAM_MCTL_2154);
}
...
static void lpsram_controller_initial_cfg(lpsram_para_t *para)
{
	...
	if (type == 1) {
		...
		if (type == 2) {
			...
			writel((readl(LPSRAM_MCTL_2200) & 0xfffffff) | (1u << 29), LPSRAM_MCTL_2200);
		} else {
			if (clk <= 204) {
				writel((readl(LPSRAM_MCTL_2150) & 0x80ff80ff) | 0x20001200, LPSRAM_MCTL_2150);
				writel((readl(LPSRAM_MCTL_2200) & 0xffffff) | (1u << 29), LPSRAM_MCTL_2200);
				writel((readl(0x02052214) & 0xffff8080) | 0x512, 0x02052214);
			} else {
				writel((readl(LPSRAM_MCTL_2130) & ~0xfu) | 4, LPSRAM_MCTL_2130);
				writel((readl(LPSRAM_MCTL_2150) & 0x80ff80ff) | 0x28001600, LPSRAM_MCTL_2150);
				writel((readl(LPSRAM_MCTL_2200) & 0xffffff) | (1u << 29), LPSRAM_MCTL_2200);
		...
		if (type >= 6) {
			{
				...
				writel((readl(LPSRAM_MCTL_2138) & 0xff808080) | lo | hi | 0xc, LPSRAM_MCTL_2138);
			}
			writel((readl(LPSRAM_MCTL_2150) & 0x80ff80ff) | 0x1e002000, LPSRAM_MCTL_2150);
			writel((readl(LPSRAM_MCTL_2200) & 0xffffff) | (1u << 29), LPSRAM_MCTL_2200);
			writel((readl(LPSRAM_MCTL_2204) & 0xffffff) | 0x13000000, LPSRAM_MCTL_2204);
			writel((readl(0x02052214) & 0xffff8080) | 0x52c, 0x02052214);
			lpsram_mr_write(
				para, 1, mr_or_default(para->dram_mr1, 193), mr_hi_or_default(para->dram_mr1, 175));
			lpsram_mr_write(para, 0, mr_or_default(para->dram_mr0, 95),
				mr_hi_or_default(para->dram_mr0, type == 8 ? 134 : 142));
			{
			...
			if (clk <= 166) {
				writel((readl(LPSRAM_MCTL_2150) & 0x80ff80ff) | 0x19001c00, LPSRAM_MCTL_2150);
				writel((readl(LPSRAM_MCTL_2200) & 0xffffff) | (1u << 29), LPSRAM_MCTL_2200);
				writel((readl(LPSRAM_MCTL_2204) & 0xffffff) | 0x13000000, LPSRAM_MCTL_2204);
				writel((readl(0x02052214) & 0xffff8080) | 0x51c, 0x02052214);
				writel(readl(LPSRAM_MCTL_2130) & ~0xfu, LPSRAM_MCTL_2130);
			} else if (clk <= 233) {
				writel((readl(LPSRAM_MCTL_2150) & 0x80ff80ff) | 0x1c001c00, LPSRAM_MCTL_2150);
				writel((readl(LPSRAM_MCTL_2200) & 0xffffff) | (1u << 29), LPSRAM_MCTL_2200);
				writel((readl(LPSRAM_MCTL_2204) & 0xffffff) | 0x13000000, LPSRAM_MCTL_2204);
				writel((readl(0x02052214) & 0xffff8080) | 0x51c, 0x02052214);
				writel((readl(LPSRAM_MCTL_2130) & ~0xfu) | 4, LPSRAM_MCTL_2130);
			...
			lpsram_mr_write(
				para, 0, mr_or_default(para->dram_mr0, 28), mr_hi_or_default(para->dram_mr0, 143));
			writel((readl(LPSRAM_MCTL_2200) & 0xffffff) | 0x60000000, LPSRAM_MCTL_2200);
		}
	}
...
static uint32_t lpsram_controller_cfg(lpsram_para_t *para)
{
	...
	if (type == 4) {
		if (clk <= 204) {
			writel((readl(LPSRAM_MCTL_2200) & 0xffffff) | (1u << 30), LPSRAM_MCTL_2200);
			switch (width_flag) {
			case 0:
				writel((readl(LPSRAM_MCTL_2204) & 0xff0000ff) | 0x121200, LPSRAM_MCTL_2204);
				break;
			case 16:
				writel((readl(LPSRAM_MCTL_2200) & 0xff0000ff) | 0x81400, LPSRAM_MCTL_2200);
				writel((readl(LPSRAM_MCTL_2204) & 0xff0000ff) | 0xa1200, LPSRAM_MCTL_2204);
				break;
			case 17:
				writel((readl(LPSRAM_MCTL_2204) & 0xff0000ff) | 0x101000, LPSRAM_MCTL_2204);
				break;
			default:
				break;
			}
			...
			case 0:
			case 17:
				writel((readl(LPSRAM_MCTL_2200) & 0xff0000ff) | 0x101c00, LPSRAM_MCTL_2200);
				writel((readl(LPSRAM_MCTL_2204) & 0xff0000ff) | 0x121a00, LPSRAM_MCTL_2204);
				break;
			case 16:
				writel((readl(LPSRAM_MCTL_2200) & 0xff0000ff) | 0x81c00, LPSRAM_MCTL_2200);
				writel((readl(LPSRAM_MCTL_2204) & 0xff0000ff) | 0xa1a00, LPSRAM_MCTL_2204);
				break;
			default:
				break;
			}
			...
			switch (width_flag) {
			case 0:
				writel((readl(LPSRAM_MCTL_2200) & 0xff0000ff) | 0x40102a00, LPSRAM_MCTL_2200);
				writel((readl(LPSRAM_MCTL_2204) & 0xff0000ff) | 0xa2700, LPSRAM_MCTL_2204);
				break;
			case 16:
				writel((readl(LPSRAM_MCTL_2200) & 0xff0000ff) | 0x82a00, LPSRAM_MCTL_2200);
				writel((readl(LPSRAM_MCTL_2204) & 0xff0000ff) | 0xa2700, LPSRAM_MCTL_2204);
				break;
			case 17:
				writel((readl(LPSRAM_MCTL_2200) & 0xff0000ff) | 0x103000, LPSRAM_MCTL_2200);
				writel((readl(LPSRAM_MCTL_2204) & 0xff0000ff) | 0xa2700, LPSRAM_MCTL_2204);
				break;
			default:
				break;
	...
	if (m_early_terminate_flag != 0) {
		writel((readl(LPSRAM_MCTL_2154) & 0x80ffffff) | (1u << 30), LPSRAM_MCTL_2154);
		writel((readl(LPSRAM_MCTL_2124) & 0xffff80ff) | (1u << 14), LPSRAM_MCTL_2124);
	}
	return 1;
}

static void lpsram_phy_cfg(void)
{
	writel(readl(LPSRAM_MAST_2014) | 0x80000000, LPSRAM_MAST_2014);
}
 ...
 */
static void lpsram_phy_dx_ui_delay_compensation(
	lpsram_para_t *para, uint32_t dx, uint32_t adj_lo, uint32_t sub_lo, uint32_t adj_hi, uint32_t sub_hi)
{
	...
	for (i = 0; i < ndx; i++) {
		...
		writel((val & 0xff000000) | new_lo | (new_hi << 8) | (new_hi << 16), reg);
	}
...
static uint32_t lpsram_core_init(lpsram_para_t *para)
{
	uint32_t msi_rc;
	uint32_t cfg_rc;
	uint32_t t;
	...
	t = para->dram_tpr4;
	if (t != 0) {
		lpsram_phy_dx_ui_delay_compensation(para, 0, t & 0x7f, (t >> 7) & 1, (t >> 8) & 0x7f, (t >> 15) & 1);
		if ((para->dram_para2 & 1) == 0)
			lpsram_phy_dx_ui_delay_compensation(
				para, 1, (t >> 16) & 0x7f, (t >> 23) & 1, (t >> 24) & 0x7f, (t >> 31) & 1);
	}
...
static int lpsram_simple_wr_test(uint32_t size_mb, uint32_t words)
{
	volatile uint32_t *lo = (volatile uint32_t *)(uintptr_t)PSRAM_MEM_BASE;
	volatile uint32_t *hi = (volatile uint32_t *)(uintptr_t)(PSRAM_MEM_BASE + ((size_mb >> 1) << 20));
	uint32_t i;
	...
	for (i = 0; i < words; i++) {
		...
		if (hv != 0xfedcba98 + i) {
			...
			printf_dram("%x != %x at address %x\n", hv, 0xfedcba98 + i, (uint32_t)(uintptr_t)&hi[i]);
			return 1;
		}
		{
			uint32_t lv = lo[i];
			...
			if (lv != 0x01234567 + i) {
				printf_dram("LPSRAM simple test FAIL.\n");
				printf_dram(
					"%x != %x at address %x\n", lv, 0x01234567 + i, (uint32_t)(uintptr_t)&lo[i]);
				return 1;
			}
		}
	}
...
static void print_dram_para(const lpsram_para_t *para)
{
	printf_dram("\n");
...
static uint32_t lpsram_init(void *buff)
{
	...
	if ((para->dram_tpr10 & 0x00080000) && !(para->dram_tpr10 & 0x80000000))
		return 0;
