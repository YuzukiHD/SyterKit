...
static void sunxi_spi_read_by_dma(sunxi_spi_t *spi, uint8_t *buf, uint32_t len)
{
	...
	flush_dcache_range((uint64_t)(uintptr_t)buf, (uint64_t)(uintptr_t)buf + len);
#endif
...
#ifdef CONFIG_ARCH_DCACHE
	invalidate_dcache_range((uint64_t)(uintptr_t)buf, (uint64_t)(uintptr_t)buf + len);
#endif
}
 ...
 * mode (`cdr_mode`). It configures the clock control register to achieve the requested
 ...
 * @param[in] cdr_mode Divider mode selected from the SPI_CDR1_MODE and
 *                     SPI_CDR2_MODE constants.
 * 
 ...
 *       and the resulting actual SPI clock frequency using `pr_debug`.
 */
 ...
 * 
 * @warning If the DMA channel cannot be requested, an error message is printed using `pr_err`.
 */
...
static inline __attribute__((always_inline)) bool sunxi_spi_gpio_available(gpio_mux_t gpio)
{
