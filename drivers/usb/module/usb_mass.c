...
/* clang-format off */
const uint8_t *sunxi_usb_mass_dev[SUNXI_USB_MASS_DEV_MAX] = {
        normal_lang_id,
...
/***************************************************************************
 * @brief Commands Unique to Direct Access Devices %%%
 ***************************************************************************/
#define SCSI_COMPARE 0x39	   /* Compare (O) */
...
/**
 * @brief Get USB descriptors for a mass-storage device.
 *
 ...
 * @param buffer Buffer that receives the descriptor data.
 *
...
/**
 * @brief Handle a USB request for the mass-storage device status.
 *
 ...
 *
 * @param req Pointer to the USB device request structure.
 * @param buffer Buffer that receives the status data.
 *
 * @return `SUNXI_USB_REQ_SUCCESSED` on success, or `SUNXI_USB_REQ_OP_ERR` on
...
/**
 * @brief Initialize the USB mass-storage driver.
 *
 ...
 *
 * @return 0 on success, or -1 if a transfer buffer cannot be allocated.
 */
static int sunxi_usb_mass_init(void) {
	...
	trans_data.base_recv_buffer = (uint8_t *) malloc(SUNXI_MASS_RECV_MEM_SIZE);
	...
	trans_data.base_send_buffer = (uint8_t *) malloc(SUNXI_MASS_SEND_MEM_SIZE);
...
/**
 * @brief Shut down the USB mass-storage driver.
 *
...
/**
 * @brief Reset the USB mass-storage driver state.
 *
...
/**
 * @brief Handle the USB mass-storage RX DMA interrupt.
 *
 ...
 *
 * @param p_arg Callback argument; unused.
 */
...
/**
 * @brief Handle the USB mass-storage TX DMA interrupt.
 *
...
/**
 * @brief Handle non-standard USB mass-storage requests.
 *
 ...
 * @param req Pointer to the USB device request structure.
 * @param buffer Request data buffer.
 * @param data_status Data-transfer status.
 * @return A USB request status code.
 */
...
/**
 * @brief Run the USB mass-storage state machine.
 *
 ...
 *
 * @param buffer Pointer to the USB transfer buffer state.
 * @return The state-machine result code.
 */
