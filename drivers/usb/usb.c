...
/**
 * @brief Handle completion of a USB DMA receive operation.
 *
 * The handler switches the endpoint back to I/O mode, receives any trailing
 * unaligned bytes, acknowledges a short packet, and invokes the active
 * device's receive callback.
 *
 * @param p_arg Argument passed to the active device callback.
 */
...
/**
 * @brief Handle completion of a USB DMA transmit operation.
 *
 * @param p_arg Argument passed to the active device callback.
 */
...
/**
 * @brief Receive pending data from the bulk OUT endpoint.
 *
 * The function services stalled and ready states, handles DMA completion, and
 * stores non-DMA data in the active receive buffer.
 *
 * @return Zero after servicing the endpoint.
 */
