...
#define pr_fmt(fmt) "rproc-sun300iw1: " fmt
...
static int sun300iw1_a27l2_start(sunxi_remoteproc_t *remoteproc)
{
	...
	setbits_le32(ccu_app + BUS_Reset1_REG,
		BUS_Reset1_REG_A27_RSTN_SW_DE_ASSERT << BUS_Reset1_REG_A27_RSTN_SW_OFFSET |
			BUS_Reset1_REG_PRESETN_TWI2_SW_DE_ASSERT << BUS_Reset1_REG_PRESETN_TWI2_SW_OFFSET);
	writel((uint32_t)remoteproc->entry, cfg + A27L_START_ADDR_REG);
	writel(0U, cfg + A27L_WFI_MODE_REG);
	setbits_le32(
		ccu_app + BUS_Reset1_REG, BUS_Reset1_REG_A27_RSTN_SW_DE_ASSERT << BUS_Reset1_REG_A27_RSTN_SW_OFFSET);
	return DRIVER_OK;
}
