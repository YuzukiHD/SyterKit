...
static int sunxi_remoteproc_resolve_entry(sunxi_remoteproc_t *remoteproc) {
	...
	if (remoteproc->format == SUNXI_REMOTEPROC_FIRMWARE_ELF32)
		remoteproc->entry =
				(uintptr_t) elf32_get_entry_addr(load_address);
	else if (remoteproc->format == SUNXI_REMOTEPROC_FIRMWARE_ELF64)
		remoteproc->entry =
				(uintptr_t) elf64_get_entry_addr(load_address);
	else
		return DRIVER_ERROR_INVALID;
	return DRIVER_OK;
}
...
int sunxi_remoteproc_load(sunxi_remoteproc_t *remoteproc) {
	...
	if (sunxi_remoteproc_validate(remoteproc) != DRIVER_OK ||
	    remoteproc->firmware[0].load_address == 0U ||
	...
	if (remoteproc->format == SUNXI_REMOTEPROC_FIRMWARE_ELF32) {
		return load_elf32_image_remap(load_address, &mapping) == 0 ?
				DRIVER_OK : DRIVER_ERROR_INVALID;
	}
	if (remoteproc->format == SUNXI_REMOTEPROC_FIRMWARE_ELF64) {
		if (remoteproc->address_map_count != 0U)
			return DRIVER_ERROR_INVALID;
		return load_elf64_image(load_address) == 0 ?
				DRIVER_OK : DRIVER_ERROR_INVALID;
	}
	return DRIVER_ERROR_INVALID;
}

static int sunxi_remoteproc_copy_raw(sunxi_remoteproc_t *remoteproc,
	...
	if (remoteproc->format != SUNXI_REMOTEPROC_FIRMWARE_RAW ||
	    remoteproc->address_map_count == 0U)
		return DRIVER_ERROR_INVALID;
...
int sunxi_remoteproc_prepare(sunxi_remoteproc_t *remoteproc) {
	if (sunxi_remoteproc_validate(remoteproc) != DRIVER_OK ||
	    sunxi_remoteproc_resolve_entry(remoteproc) != DRIVER_OK)
		return DRIVER_ERROR_INVALID;
	return remoteproc->ops->prepare == NULL ?
			DRIVER_OK : remoteproc->ops->prepare(remoteproc);
}

int sunxi_remoteproc_start(sunxi_remoteproc_t *remoteproc) {
	if (sunxi_remoteproc_validate(remoteproc) != DRIVER_OK ||
	    remoteproc->ops->start == NULL)
		return DRIVER_ERROR_INVALID;
