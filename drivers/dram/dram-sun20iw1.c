...
#define pr_fmt(fmt) "dram-sun20iw1: " fmt

/**
 * @file dram-sun20iw1.c
 * @brief DRAM controller driver for the Allwinner sun20iw1 SoC.
 *
 * In-tree DRAM controller initialization for the sun20iw1 SoC: sets the DDR
...
static void sid_read_ldoB_cal(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	reg = (readl(dram->registers.sid.base + SYS_LDOB_SID) & 0xff00) >> 8;
...
static void dram_voltage_set(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	clrsetbits_le32((dram->registers.sysctrl.base + LDO_CTAL_REG), 0x20ff00, vol << 8);
...
static void dram_enable_all_master(const sunxi_dram_t *dram)
{
	...
	writel(0xff, (dram->registers.mctl_com.base + MCTL_COM_MAER1));
	writel(0xffff, (dram->registers.mctl_com.base + MCTL_COM_MAER2));
	udelay(10);
}
...
static void dram_disable_all_master(const sunxi_dram_t *dram)
{
	...
	writel(0, (dram->registers.mctl_com.base + MCTL_COM_MAER1));
	writel(0, (dram->registers.mctl_com.base + MCTL_COM_MAER2));
	udelay(10);
}
...
static void eye_delay_compensation(const sunxi_dram_t *dram, dram_para_t *para) // s1
{
	...
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DATX0IOCR(9)), delay); // DQS0 P
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DATX0IOCR(10)), delay); // DQS0 N

	// DQS1 read and write delay
	delay = (para->dram_tpr11 & 0xf00000) >> 11;
	delay |= (para->dram_tpr12 & 0xf00000) >> 19;
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DATX1IOCR(9)), delay); // DQS1 P
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DATX1IOCR(10)), delay); // DQS1 N
	...
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DXnSDLR6(1)), (para->dram_tpr11 & 0xf00000) << 5);

	// PGCR0: release AC loopback FIFO reset
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR0), (1 << 26));
	...
	for (i = 6; i < 27; ++i)
		setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_ACIOCR1(i)), delay);

	// Set CK CS delay
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_ACIOCR1(2)), (para->dram_tpr10 & 0x0f) << 8);
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_ACIOCR1(3)), (para->dram_tpr10 & 0x0f) << 8);
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_ACIOCR1(28)), (para->dram_tpr10 & 0xf00) >> 4);
}
...
static void mctl_set_timing_params(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	/* Set mode registers */
	writel(mr0, (dram->registers.mctl_phy.base + MCTL_PHY_DRAM_MR0));
	writel(mr1, (dram->registers.mctl_phy.base + MCTL_PHY_DRAM_MR1));
	writel(mr2, (dram->registers.mctl_phy.base + MCTL_PHY_DRAM_MR2));
	writel(mr3, (dram->registers.mctl_phy.base + MCTL_PHY_DRAM_MR3));
	...
	writel((trefi << 16) | (trfc << 0), (dram->registers.mctl_phy.base + MCTL_PHY_RFSHTMG));
	writel((trefi << 15) & 0x0fff0000, (dram->registers.mctl_phy.base + MCTL_PHY_RFSHCTL1));
}
...
static int ccu_set_pll_ddr_clk(const sunxi_dram_t *dram, int index, dram_para_t *para)
{
	...
	val = readl((dram->registers.ccu.base + CCU_PLL_DDR_CTRL_REG));
	val &= 0xfff800fc; // clear dividers
	val |= (n - 1) << 8; // set PLL division
	val |= 0xc0000000; // enable PLL and LDO
	val &= 0xdfffffff;
	writel(val | 0x20000000, (dram->registers.ccu.base + CCU_PLL_DDR_CTRL_REG));

	// wait for PLL to lock
	while ((readl((dram->registers.ccu.base + CCU_PLL_DDR_CTRL_REG)) & 0x10000000) == 0) {
		;
	}
	...
	// enable PLL output
	val = readl(dram->registers.ccu.base);
	val |= 0x08000000;
	writel(val, dram->registers.ccu.base);

	// turn clock gate on
	val = readl((dram->registers.ccu.base + CCU_DRAM_CLK_REG));
	val &= 0xfcfffcfc; // select DDR clk source, n=1, m=1
	val |= 0x80000000; // turn clock on
	writel(val, (dram->registers.ccu.base + CCU_DRAM_CLK_REG));
...
{
	// assert MBUS reset
	clrbits_le32((dram->registers.ccu.base + CCU_MBUS_CLK_REG), (1 << 30));

	// turn off sdram clock gate, assert sdram reset
	clrbits_le32((dram->registers.ccu.base + CCU_DRAM_BGR_REG), 0x10001);
	clrsetbits_le32((dram->registers.ccu.base + CCU_DRAM_CLK_REG), (1 << 31) | (1 << 30), (1 << 27));
	udelay(10);

	// set ddr pll clock
	para->dram_clk = ccu_set_pll_ddr_clk(dram, 0, para) / 2;
	udelay(100);
	dram_disable_all_master(dram);

	// release sdram reset
	setbits_le32((dram->registers.ccu.base + CCU_DRAM_BGR_REG), (1 << 16));

	// release MBUS reset
	setbits_le32((dram->registers.ccu.base + CCU_MBUS_CLK_REG), (1 << 30));
	setbits_le32((dram->registers.ccu.base + CCU_DRAM_CLK_REG), (1 << 30));

	udelay(5);

	// turn on sdram clock gate
	setbits_le32((dram->registers.ccu.base + CCU_DRAM_BGR_REG), (1 << 0));

	// turn dram clock gate on, trigger sdr clock update
	setbits_le32((dram->registers.ccu.base + CCU_DRAM_CLK_REG), (1 << 31) | (1 << 27));
	udelay(5);

	// mCTL clock enable
	writel(0x8000, (dram->registers.mctl_phy.base + MCTL_PHY_CLKEN));
	udelay(10);
}
...
static void mctl_com_init(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	clrsetbits_le32((dram->registers.mctl_com.base + MCTL_COM_DBGCR), 0x3f00, 0x2000);
	...
	writel(val, (dram->registers.mctl_com.base + MCTL_COM_WORK_MODE0));
	...
	val = (readl((dram->registers.mctl_com.base + MCTL_COM_WORK_MODE0)) & 0x1) ? 0x303 : 0x201;
	writel(val, (dram->registers.mctl_phy.base + MCTL_PHY_ODTMAP));

	// set mctl reg 3c4 to zero when using half DQ
	if (para->dram_para2 & (1 << 0))
		writel(0, (dram->registers.mctl_phy.base + MCTL_PHY_DXnGCR0(1)));

	// set dram address mapping from dram_tpr4 param
	if (para->dram_tpr4) {
		setbits_le32((dram->registers.mctl_com.base + MCTL_COM_WORK_MODE0), (para->dram_tpr4 & 0x3) << 25);
		setbits_le32((dram->registers.mctl_com.base + MCTL_COM_WORK_MODE1), (para->dram_tpr4 & 0x7fc) << 10);
	}
}
...
static const uint8_t ac_remapping_tables[][22] = {
	...
	/* ASIC Chip */
	[1] = { 0x1, 0x9, 0x3, 0x7, 0x8, 0x12, 0x4, 0xD, 0x5, 0x6, 0xA, 0x2, 0xE, 0xC, 0x0, 0x0, 0x15, 0x11, 0x14, 0x13,
		0xB, 0x16 }, // Generic DDR3 Type1
	[2] = { 0x4, 0x9, 0x3, 0x7, 0x8, 0x12, 0x1, 0xD, 0x2, 0x6, 0xA, 0x5, 0xE, 0xC, 0x0, 0x0, 0x15, 0x11, 0x14, 0x13,
		0xB, 0x16 }, // Generic DDR3 Type C
	[3] = { 0x1, 0x7, 0x8, 0xC, 0xA, 0x12, 0x4, 0xD, 0x5, 0x6, 0x3, 0x2, 0x9, 0x0, 0x0, 0x0, 0x15, 0x11, 0x14, 0x13,
		0xB, 0x16 }, // Generic DDR3 Type 8
	[4] = { 0x4, 0xC, 0xA, 0x7, 0x8, 0x12, 0x1, 0xD, 0x2, 0x6, 0x3, 0x5, 0x9, 0x0, 0x0, 0x0, 0x15, 0x11, 0x14, 0x13,
		0xB, 0x16 }, // Generic DDR3 Type 9
	[5] = { 0xD, 0x2, 0x7, 0x9, 0xC, 0x13, 0x5, 0x1, 0x6, 0x3, 0x4, 0x8, 0xA, 0x0, 0x0, 0x0, 0x15, 0x16, 0x12, 0x11,
		0xB, 0x14 }, // DDR3 Type bf (T113)
	[6] = { 0x3, 0xA, 0x7, 0xD, 0x9, 0xB, 0x1, 0x2, 0x4, 0x6, 0x8, 0x5, 0xC, 0x0, 0x0, 0x0, 0x14, 0x12, 0x0, 0x15,
		0x16, 0x11 }, // DDR2
	[7] = { 0x3, 0x2, 0x4, 0x7, 0x9, 0x1, 0x11, 0xC, 0x12, 0xE, 0xD, 0x8, 0xF, 0x6, 0xA, 0x5, 0x13, 0x16, 0x10,
		0x15, 0x14, 0xB }, // DDR3 D1-H
	[8] = { 0x2, 0x13, 0x8, 0x6, 0xE, 0x5, 0x14, 0xA, 0x3, 0x12, 0xD, 0xB, 0x7, 0xF, 0x9, 0x1, 0x16, 0x15, 0x11,
		0xC, 0x4, 0x10 }, // DDR3 H133
	[9] = { 0x1, 0x2, 0xD, 0x8, 0xF, 0xC, 0x13, 0xA, 0x3, 0x15, 0x6, 0x11, 0x9, 0xE, 0x5, 0x10, 0x14, 0x16, 0xB,
		0x7, 0x4, 0x12 }, // DDR2 H133
};
 ...
 *
 * Selects one of the AC remapping tables based on the DDR type, efuse and
 * timing parameters, and programs the controller REMAP0-3 registers. Only
 * DDR2/DDR3 support remap.
 *
 * @param[in] dram DRAM controller register block.
 * @param[in] para DRAM parameters providing the DRAM type.
 */
static void mctl_phy_ac_remapping(const sunxi_dram_t *dram, dram_para_t *para)
{
   ...
   */
	...
	writel(val, (dram->registers.mctl_com.base + MCTL_COM_REMAP0));
	val = (cfg[7] << 10) | (32 * cfg[6]) | cfg[5] | (cfg[8] << 15) | (cfg[9] << 20) | (cfg[10] << 25);
	writel(val, (dram->registers.mctl_com.base + MCTL_COM_REMAP1));
	val = (cfg[13] << 10) | (32 * cfg[12]) | cfg[11] | (cfg[14] << 15) | (cfg[15] << 20);
	writel(val, (dram->registers.mctl_com.base + MCTL_COM_REMAP2));
	val = (cfg[18] << 10) | (32 * cfg[17]) | cfg[16] | (cfg[19] << 15) | (cfg[20] << 20) | (cfg[21] << 25);
	writel(val, (dram->registers.mctl_com.base + MCTL_COM_REMAP3));

	pr_trace("MCTL_COM_REMAP0 = 0x%x\n", readl((dram->registers.mctl_com.base + MCTL_COM_REMAP0)));
	pr_trace("MCTL_COM_REMAP1 = 0x%x\n", readl((dram->registers.mctl_com.base + MCTL_COM_REMAP1)));
	pr_trace("MCTL_COM_REMAP2 = 0x%x\n", readl((dram->registers.mctl_com.base + MCTL_COM_REMAP2)));
	pr_trace("MCTL_COM_REMAP3 = 0x%x\n", readl((dram->registers.mctl_com.base + MCTL_COM_REMAP3)));
}
...
static unsigned int mctl_channel_init(const sunxi_dram_t *dram, unsigned int ch_index, dram_para_t *para)
{
	...
	clrsetbits_le32((dram->registers.mctl_com.base + MCTL_COM_TMR), 0xfff, (para->dram_clk / 2) - 1);

	// MRCTRL0 nibble 3 undocumented
	clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR2), 0xf00, 0x300);
	...
	if (para->dram_clk > 672)
		clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DXnGCR0(0)), 0xf63e, val);
	else
		clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DXnGCR0(0)), 0xf03e, val);

	// DX1GCR0
	if (para->dram_clk > 672) {
		setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DXnGCR0(0)), 0x400);
		clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DXnGCR0(1)), 0xf63e, val);
	} else {
		clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DXnGCR0(1)), 0xf03e, val);
	}

	// (dram->registers.mctl_phy.base+MCTL_PHY_ACIOCR0) undocumented
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_ACIOCR0), (1 << 1));

	eye_delay_compensation(dram, para);

	// set PLL SSCG ?
	val = readl((dram->registers.mctl_phy.base + MCTL_PHY_PGCR2));
	if (dqs_gating_mode == 1) {
		clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR2), 0xc0, 0);
		clrbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DQSGMR), 0x107);
		...
		udelay(10);
		setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR2), 0xc0);
	}

	if (para->dram_type == SUNXI_DRAM_TYPE_LPDDR2 || para->dram_type == SUNXI_DRAM_TYPE_LPDDR3) {
		if (dqs_gating_mode == 1)
			clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DXCCR), 0x080000c0, 0x80000000);
		else
	...
	if (dqs_gating_mode == 1) {
		...
		if ((readl((dram->registers.r_cpucfg.base + SUNXI_R_CPUCFG_SUP_STAN_FLAG)) & (1 << 16)) == 0) {
			...
			writel(0x62, (dram->registers.mctl_phy.base + MCTL_PHY_PIR));
		}
	}
	...
	if (readl((dram->registers.r_cpucfg.base + SUNXI_R_CPUCFG_SUP_STAN_FLAG)) & (1 << 16)) {
		...
		clrbits_le32((dram->registers.r_prcm.base + VDD_SYS_PWROFF_GATING_REG), 0x1);
		udelay(10);
		...
		if (dqs_gating_mode == 1) {
			clrbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR2), 0xc0);
			clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR3), 0x06000000, 0x02000000);
			udelay(1);
	...
	udelay(10);
	clrbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_RFSHCTL0), (1 << 31));
	udelay(10);
	setbits_le32((dram->registers.mctl_com.base + MCTL_COM_CCCR), (1 << 31));
	udelay(10);

	clrbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR3), 0x06000000);

	if (dqs_gating_mode == 1)
		clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_DXCCR), 0xc0, 0x40);
...
static unsigned int DRAMC_get_dram_size(const sunxi_dram_t *dram)
{
	...
	val = readl((dram->registers.mctl_com.base + MCTL_COM_WORK_MODE0)); /* MCTL_COM_WORK_MODE0 */
...
static int dqs_gate_detect(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	pr_debug("DX0 state: %lu\n", dx0);
	pr_debug("DX1 state: %lu\n", dx1);
...
static void mctl_vrefzq_init(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_IOVCR0), 0x7f7f7f7f, para->dram_tpr5);

	// IOCVR1
	if ((para->dram_tpr13 & (1 << 16)) == 0)
		clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_IOVCR1), 0x7f, para->dram_tpr6 & 0x7f);
}
...
static int mctl_core_init(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	mctl_com_init(dram, para);

	mctl_phy_ac_remapping(dram, para);

	mctl_set_timing_params(dram, para);

	return mctl_channel_init(dram, 0, para);
}
...
static int auto_scan_dram_size(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	for (current_rank = 0; current_rank < rank_count; current_rank++) {
		mc_work_mode = ((dram->registers.mctl_com.base + MCTL_COM_WORK_MODE0) + 4 * current_rank);

		/* Set 16 Row 4Bank 512BPage for Rank 1 */
		if (current_rank == 1) {
			clrsetbits_le32((dram->registers.mctl_com.base + MCTL_COM_WORK_MODE0), 0xf0c, 0x6f0);
			clrsetbits_le32((dram->registers.mctl_com.base + MCTL_COM_WORK_MODE1), 0xf0c, 0x6f0);
			/* update Rank 1 addr */
			rank1_addr = base + (0x1 << 27);
		}
		...
		for (i = 11; i < 17; i++) {
			ret = base + (1 << (i + 2 + 9)); /* row-bank-column */
			cnt = 0;
			for (j = 0; j < 64; j++) {
		...
		if (current_rank == 1) { /* Set bank mode for rank0 */
			clrsetbits_le32((dram->registers.mctl_com.base + MCTL_COM_WORK_MODE0), 0xffc, 0x6a4);
		}
...
static int auto_scan_dram_rank_width(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	mctl_core_init(dram, para);

	if (readl((dram->registers.mctl_phy.base + MCTL_PHY_PGSR0)) & (1 << 20))
		return 0;

	if (dqs_gate_detect(dram, para) == 0)
		return 0;
...
static int init_DRAM(sunxi_dram_t *dram, int type, dram_para_t *para)
{
	...
	if (para->dram_tpr13 & (1 << 16)) {
		...
		clrbits_le32((dram->registers.sysctrl.base + ZQ_CAL_CTRL_REG), 0x3);
		writel(para->dram_tpr13 & (1 << 16), (dram->registers.r_prcm.base + ANALOG_PWROFF_GATING_REG));
		udelay(10);
		clrsetbits_le32((dram->registers.sysctrl.base + ZQ_CAL_CTRL_REG), 0x108, (1 << 1));
		udelay(10);
		setbits_le32((dram->registers.sysctrl.base + ZQ_CAL_CTRL_REG), (1 << 0));
		udelay(20);
		pr_debug("ZQ value = 0x%08x\n", readl((dram->registers.sysctrl.base + ZQ_RES_STATUS_REG)));
	}

	dram_voltage_set(dram, para);
	...
	if (para->dram_tpr13 & (1 << 9)) {
		clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR0), 0xf000, 0x5000);
	} else {
		if (para->dram_type != SUNXI_DRAM_TYPE_LPDDR2)
			clrbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR0), 0xf000);
	}

	/* Disable ZQ calibration */
	setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_ZQCR), (1 << 31));
	...
	/* Set PAD Hold */
	if (para->dram_tpr13 & (1 << 16))
		clrbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR2), (1 << 13));
	else
		setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_PGCR2), (1 << 13));

	/* Set LPDDR3 ODT delay */
	if (para->dram_type == SUNXI_DRAM_TYPE_LPDDR3)
		clrsetbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_ODTCFG), 0xf0000, 0x1000);
