...
static void eye_delay_compensation(const sunxi_dram_t *dram, dram_para_t *para)// s1
{
	...
	for (i = 6; i < 27; ++i) setbits_le32((dram->registers.mctl_phy.base + MCTL_PHY_ACIOCR1(i)), delay);
...
static int dramc_simple_wr_test(const sunxi_dram_t *dram, uint32_t mem_mb,
				int len) {
...
static int init_DRAM(sunxi_dram_t *dram, int type, dram_para_t *para) {
	...
	if (para->dram_tpr13 & (1 << 28)) {
		if ((readl(dram->rtc.data_base + 0xf8U) & (1 << 16)) ||
...
DT2C_DRIVER_COMPAT("allwinner,sun8iw21-dram");
