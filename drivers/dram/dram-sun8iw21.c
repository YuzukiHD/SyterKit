...
#define pr_fmt(fmt) "dram-sun8iw21: " fmt

/**
 * @file dram-sun8iw21.c
 * @brief DRAM controller driver for the Allwinner sun8iw21 SoC.
 *
 * In-tree DRAM controller initialization for the sun8iw21 SoC: computes the
 * timing parameters, configures the controller and PHY, auto-scans the SDRAM
 * topology and size, and runs a memory write/read test.
 */
...
#include <drivers/dram/sun8iw21/reg.h>
...
static void mctl_sys_init(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	clrsetbits_le32((dram->registers.ccu.base + CCU_DRAM_CLK_REG), (1 << 31), (1 << 27));
	sdelay(10);
 ...
 *
 * Selects one of the AC remapping tables based on the DDR type and efuse, and
 * programs the controller REMAP0-3 registers. Only DDR2/DDR3 support remap.
 *
 * @param[in] dram DRAM controller register block.
 ...
 */
static uint32_t mctl_channel_init(const sunxi_dram_t *dram, uint32_t ch_index, dram_para_t *para)
{
 ...
 */
static uint32_t dramc_get_dram_size(const sunxi_dram_t *dram)
{
	uint32_t val;
	uint32_t size;
...
static int dqs_gate_detect(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	pr_debug("DX0 state: %d\n", dx0);
	pr_debug("DX1 state: %d\n", dx1);
 ...
 */
static int dramc_simple_wr_test(const sunxi_dram_t *dram, uint32_t mem_mb, int len)
{
	...
	addr = (uint32_t *)dram->memory_base;
...
static int auto_scan_dram_size(const sunxi_dram_t *dram, dram_para_t *para)
{
	...
	for (current_rank = 0; current_rank < rank_count; current_rank++) {
		...
		pr_debug("rank %d page size = %d KB \n", current_rank, i);
 ...
 *
 * Configures the ZQ controller, optionally auto-scans the DRAM topology,
 * performs the final core initialization, determines the DRAM size, applies
 * the refresh, ODT and VTF settings and runs a memory test if requested.
 *
 * @param[in] dram DRAM controller register block.
...
static int init_DRAM(sunxi_dram_t *dram, int type, dram_para_t *para)
{
	...
	if (para->dram_tpr13 & (1 << 28)) {
		if ((readl(dram->rtc.data_base + 0xf8U) & (1 << 16)) || dramc_simple_wr_test(dram, mem_size_mb, 4096))
			return 0;
	}
