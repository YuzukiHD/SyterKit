 ...
 * @return Value with its two bytes exchanged.
 */
...
static inline u32_t __swab32(u32_t x) {
	return ((x << 24) | (x >> 24) | ((x & (u32_t) 0x0000ff00UL) << 8) | ((x & (u32_t) 0x00ff0000UL) >> 8));
}

/**
 * @brief Reverse the byte order of a 64-bit value.
 * @param[in] x Value to convert.
 * @return Value with its eight bytes reversed.
 */
static inline u64_t __swab64(u64_t x) {
	return ((x << 56) | (x >> 56) | ((x & (u64_t) 0x000000000000ff00ULL) << 40) | ((x & (u64_t) 0x0000000000ff0000ULL) << 24) | ((x & (u64_t) 0x00000000ff000000ULL) << 8) |
			((x & (u64_t) 0x000000ff00000000ULL) >> 8) | ((x & (u64_t) 0x0000ff0000000000ULL) >> 24) | ((x & (u64_t) 0x00ff000000000000ULL) >> 40));
}

/**
 * @brief Exchange the two 16-bit half-words of a 32-bit value.
 * @param[in] x Value to convert.
 * @return Value with its half-words exchanged.
 */
...
/**
 * @brief Exchange the two bytes in each half-word of a 32-bit value.
 * @param[in] x Value to convert.
 * @return Value with the bytes in each half-word exchanged.
 */
