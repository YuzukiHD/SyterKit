...
.macro SAVE_ALL csrw mscratch, sp addi sp, sp, -PT_REGS_SIZE SREG zero, 0 * REGSZ(sp)
SREG x1, 1 * REGSZ(sp)
SREG x3, 3 * REGSZ(sp)
SREG x4, 4 * REGSZ(sp)
SREG x5, 5 * REGSZ(sp)
...
csrrw t0, mscratch, zero csrr s0, mstatus csrr t1, mepc csrr t2, mtval csrr t3, mcause SREG t0, 2 * REGSZ(sp)
SREG s0, 32 * REGSZ(sp)
SREG t1, 33 * REGSZ(sp)
SREG t2, 34 * REGSZ(sp)
SREG t3, 35 * REGSZ(sp)
li t0, -1 SREG t0, 36 * REGSZ(sp)
mv a0,
	sp.endm

		.macro RESTORE_ALL csrr a0,
	mscratch LREG t0, 32 * REGSZ(a0)
csrw mstatus, t0 LREG t0, 33 * REGSZ(a0)
csrw mepc, t0 LREG x1, 1 * REGSZ(a0)
LREG x2, 2 * REGSZ(a0)
LREG x3, 3 * REGSZ(a0)
LREG x4, 4 * REGSZ(a0)
LREG x5, 5 * REGSZ(a0)
...
csrw mscratch, zero LREG x10, 10 * REGSZ(a0) mret.endm
