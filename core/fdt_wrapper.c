...
{
	const char *cp; /* temporary char pointer */
	const char *newp; /* temporary newval char pointer */
	char *endp;
	...
	*len = 0;
	...
	if (*newp == '<') {
		...
		while ((stridx < count) && (*newp != ']')) {
			...
			if (!isxdigit(*newp))
				break;
			tmp = simple_strtoul(newp, &endp, 16);
			newp = endp;
			*data++ = tmp & 0xFF;
			*len = *len + 1;
		}
	...
	if ((len % 4) == 0) {
		if (len > CMD_FDT_MAX_DUMP)
			...
			for (j = 0, p = data; j < len / 4; j++)
				printk(LOG_LEVEL_MUTE, "0x%08x%s", fdt32_to_cpu(p[j]), j < (len / 4 - 1) ? " " : "");
			printk(LOG_LEVEL_MUTE, ">");
		}
