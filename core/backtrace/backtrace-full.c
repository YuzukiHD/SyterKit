...
extern const struct backtrace_symbol __backtrace_symbols[]
	__attribute__((weak));
 ...
 */
static const struct backtrace_symbol *backtrace_find_symbol(uintptr_t address,
						     uintptr_t *size) {
	...
	if (!__backtrace_symbols || !&__backtrace_symbol_count ||
...
void backtrace_print_frame(uintptr_t raw_address) {
	...
	printk(LOG_LEVEL_BACKTRACE, " [<0x%016lx>] %s+0x%lx/0x%lx\n",
	       (unsigned long) address,
	       __backtrace_symbol_names + symbol->name_offset,
	...
	printk(LOG_LEVEL_BACKTRACE, " [<0x%08x>] %s+0x%x/0x%x\n",
	       (uint32_t) address,
	       __backtrace_symbol_names + symbol->name_offset,
	       (uint32_t) (address - symbol->address), (uint32_t) size);
#endif
}
