...
static int number_digits(unsigned long long value, unsigned base,
				char *buffer, bool upper)
{
...
unsigned vformat(format_putc_t putc, void *arg,
			       const char *fmt, va_list args)
{
	...
	while (*fmt) {
		...
		for (;;) {
			switch (*fmt) {
			case '-': left = true; fmt++; continue;
			case '0': zero = true; fmt++; continue;
			case '+': plus = true; fmt++; continue;
			case ' ': space = true; fmt++; continue;
			case '#': alternate = true; fmt++; continue;
			default: break;
			}
			break;
		}
		...
		else if (*fmt == 't') { length = 4; fmt++; }
		else if (*fmt == 'j') { length = 2; fmt++; }

		{
			char spec = *fmt++;
		switch (spec) {
		case '%': emit(&state, '%'); continue;
		case 'c':
			value = (unsigned char) va_arg(args, int);
			padding = width > 1 ? width - 1 : 0;
			if (!left) emit_repeat(&state, ' ', padding);
		...
		case 's':
			...
			if (!left) emit_repeat(&state, ' ', padding);
			for (int i = 0; i < digits; i++) emit(&state, str[i]);
			if (left) emit_repeat(&state, ' ', padding);
			continue;
		case 'p':
		...
		case 'd': case 'i': case 'u': case 'o': case 'x': case 'X':
			if (spec == 'd' || spec == 'i') {
				...
				if (length == 1) signed_value = va_arg(args, long);
				else if (length == 2) signed_value = va_arg(args, long long);
				else if (length == 3) signed_value = (long long)va_arg(args, size_t);
				else signed_value = va_arg(args, int);
				if (signed_value < 0) { negative = true; value = (unsigned long long)(-(signed_value + 1)) + 1; }
				else value = (unsigned long long)signed_value;
			} else if (spec != 'p') {
				if (length == 1) value = va_arg(args, unsigned long);
				else if (length == 2) value = va_arg(args, unsigned long long);
				else if (length == 3) value = va_arg(args, size_t);
			...
			}
			{
				...
				base_prefix = 0;
				if (alternate && base == 16 && (value || spec == 'p')) base_prefix = 2;
				else if (alternate && base == 8 && (value || precision == 0)) base_prefix = 1;
				sign_prefix = negative || plus || space;
				prefix = sign_prefix + base_prefix;
				if (zero && !left && precision < 0 && width > prefix + digits) padding = width - prefix - digits;
				else if (width > prefix + digits + padding) padding += width - prefix - digits - padding;
				if (!left && !(zero && precision < 0))
					emit_repeat(&state, ' ', padding - precision_zeros);
				if (negative) emit(&state, '-'); else if (plus) emit(&state, '+'); else if (space) emit(&state, ' ');
				if (base_prefix == 2) { emit(&state, '0'); emit(&state, upper ? 'X' : 'x'); }
				else if (base_prefix == 1) emit(&state, '0');
				if (zero && !left && precision < 0)
					emit_repeat(&state, '0', padding);
				else
					emit_repeat(&state, '0', precision_zeros);
				for (int i = digits - 1; i >= 0; i--) emit(&state, number[i]);
				if (left) emit_repeat(&state, ' ', padding - precision_zeros);
			}
			continue;
		default:
			emit(&state, '%');
...
unsigned format(format_putc_t putc, void *arg,
			      const char *fmt, ...)
{
	va_list args;
	unsigned count;
