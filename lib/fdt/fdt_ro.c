...
const char *fdt_get_string(const void *fdt, int stroffset, int *lenp) {
	...
	err = totalsize;
	if (totalsize < 0) goto fail;

	err = -FDT_ERR_BADOFFSET;
	absoffset = stroffset + fdt_off_dt_strings(fdt);
	if (absoffset >= (unsigned) totalsize) goto fail;
	len = totalsize - absoffset;

	if (fdt_magic(fdt) == FDT_MAGIC) {
		if (stroffset < 0) goto fail;
		if (can_assume(LATEST) || fdt_version(fdt) >= 17) {
			if ((unsigned) stroffset >= fdt_size_dt_strings(fdt)) goto fail;
			if ((fdt_size_dt_strings(fdt) - stroffset) < len) len = fdt_size_dt_strings(fdt) - stroffset;
		}
	} else if (fdt_magic(fdt) == FDT_SW_MAGIC) {
		unsigned int sw_stroffset = -stroffset;

		if ((stroffset >= 0) || (sw_stroffset > fdt_size_dt_strings(fdt))) goto fail;
...
int fdt_find_max_phandle(const void *fdt, uint32_t *phandle) {
	...
	while (true) {
		...
		if (offset < 0) {
			if (offset == -FDT_ERR_NOTFOUND) break;
...
int fdt_generate_phandle(const void *fdt, uint32_t *phandle) {
	...
	if (max == FDT_MAX_PHANDLE) return -FDT_ERR_NOPHANDLES;

	if (phandle) *phandle = max + 1;
...
static const struct fdt_reserve_entry *fdt_mem_rsv(const void *fdt, int n) {
	...
	if (!can_assume(VALID_INPUT)) {
		if (absoffset < fdt_off_mem_rsvmap(fdt)) return NULL;
...
int fdt_get_mem_rsv(const void *fdt, int n, uint64_t *address, uint64_t *size) {
	...
	if (!can_assume(VALID_INPUT) && !re) return -FDT_ERR_BADOFFSET;
...
int fdt_num_mem_rsv(const void *fdt) {
	...
	for (i = 0; (re = fdt_mem_rsv(fdt, i)) != NULL; i++) {
		if (fdt64_ld_(&re->size) == 0) return i;
	}
...
static int nextprop_(const void *fdt, int offset) {
	...
	do {
		...
		switch (tag) {
			case FDT_END:
				if (nextoffset >= 0) return -FDT_ERR_BADSTRUCTURE;
				else
					return nextoffset;
...
int fdt_subnode_offset_namelen(const void *fdt, int offset, const char *name, int namelen) {
	...
	for (depth = 0; (offset >= 0) && (depth >= 0); offset = fdt_next_node(fdt, offset, &depth))
		if ((depth == 1) && fdt_nodename_eq_(fdt, offset, name, namelen)) return offset;
...
const char *fdt_get_name(const void *fdt, int nodeoffset, int *len) {
	...
	if (((err = fdt_ro_probe_(fdt)) < 0) || ((err = fdt_check_node_offset_(fdt, nodeoffset)) < 0)) goto fail;
...
static const struct fdt_property *fdt_get_property_by_offset_(const void *fdt, int offset, int *lenp) {
	...
	if (!can_assume(VALID_INPUT) && (err = fdt_check_prop_offset_(fdt, offset)) < 0) {
		if (lenp) *lenp = err;
		return NULL;
	}
	...
	if (lenp) *lenp = fdt32_ld_(&prop->len);
...
const void *fdt_getprop_namelen(const void *fdt, int nodeoffset, const char *name, int namelen, int *lenp) {
	...
	/* Handle realignment */
	if (!can_assume(LATEST) && fdt_version(fdt) < 0x10 && (poffset + sizeof(*prop)) % 8 && fdt32_ld_(&prop->len) >= 8) return prop->data + 4;
	return prop->data;
}
...
const void *fdt_getprop_by_offset(const void *fdt, int offset, const char **namep, int *lenp) {
	...
	/* Handle realignment */
	if (!can_assume(LATEST) && fdt_version(fdt) < 0x10 && (offset + sizeof(*prop)) % 8 && fdt32_ld_(&prop->len) >= 8) return prop->data + 4;
	return prop->data;
}
...
int fdt_get_path(const void *fdt, int nodeoffset, char *buf, int buflen) {
	...
	if ((offset == -FDT_ERR_NOTFOUND) || (offset >= 0)) return -FDT_ERR_BADOFFSET;
...
int fdt_supernode_atdepth_offset(const void *fdt, int nodeoffset, int supernodedepth, int *nodedepth) {
	...
	for (offset = 0, depth = 0; (offset >= 0) && (offset <= nodeoffset); offset = fdt_next_node(fdt, offset, &depth)) {
		...
		if (offset == nodeoffset) {
			...
			if (supernodedepth > depth) return -FDT_ERR_NOTFOUND;
			else
...
int fdt_node_depth(const void *fdt, int nodeoffset) {
	...
	if (err) return (can_assume(LIBFDT_FLAWLESS) || err < 0) ? err : -FDT_ERR_INTERNAL;
	return nodedepth;
}
...
int fdt_node_offset_by_prop_value(const void *fdt, int startoffset, const char *propname, const void *propval, int proplen) {
	...
	for (offset = fdt_next_node(fdt, startoffset, NULL); offset >= 0; offset = fdt_next_node(fdt, offset, NULL)) {
		...
		if (val && (len == proplen) && (memcmp(val, propval, len) == 0)) return offset;
	}
...
int fdt_node_offset_by_compatible(const void *fdt, int startoffset, const char *compatible) {
	...
	for (offset = fdt_next_node(fdt, startoffset, NULL); offset >= 0; offset = fdt_next_node(fdt, offset, NULL)) {
		...
		if ((err < 0) && (err != -FDT_ERR_NOTFOUND)) return err;
		else if (err == 0)
			return offset;
	}
