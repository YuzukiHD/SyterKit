...
SyterKit keeps the useful part of the SPL-2.0 design: a short, visible boot
sequence and small hardware interfaces. It does not use a runtime driver/device
registry. Every application owns its device instances, chooses the SoC driver
...
3. `boards/<board>/<app>/main.c` is the composition layer. It decides the order
   and error policy for that application, just as SPL-2.0's board code decides
   the order of its HAL calls.
...
## What to borrow from SPL-2.0

SPL-2.0's useful pattern is a thin stage wrapper around explicit calls:
...
implementations of one protocol. SPL-2.0's linker lists are useful only for
