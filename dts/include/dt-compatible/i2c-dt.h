...
static inline __attribute__((always_inline)) int sunxi_i2c_dt_read_config(sunxi_i2c_t *i2c, int node)
{
	...
	config.i2c_clk.gate_reg_base = (uintptr_t)dt2c_fdt32_to_cpu(clock_gate[0]);
...
static inline __attribute__((always_inline)) int sunxi_i2c_dt_read_alias(sunxi_i2c_t *i2c, const char *alias)
{
	return sunxi_i2c_dt_read_config(i2c, syterkit_dt_alias_node(alias, SUNXI_I2C_COMPATIBLE));
}
