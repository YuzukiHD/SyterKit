...
static inline __attribute__((always_inline)) bool sunxi_serial_dt_node_available(int node)
{
	return syterkit_dt_node_available(node);
}

static inline __attribute__((always_inline)) int sunxi_serial_dt_stdout_node(void)
{
	...
	if (chosen < 0)
		return chosen;
	stdout_path = (const char *)dt2c_fdt_getprop(DT2C_FDT_COMPILED_TREE, chosen, "stdout-path", &length);
...
static inline __attribute__((always_inline)) int sunxi_serial_dt_read_config(sunxi_serial_t *uart, int node)
{
	...
	config.uart_clk.gate_reg_base = (uintptr_t)dt2c_fdt32_to_cpu(clock_gate[0]);
	...
	parity = (const char *)dt2c_fdt_getprop(DT2C_FDT_COMPILED_TREE, node, "parity", &length);
...
static inline __attribute__((always_inline)) int sunxi_serial_dt_read_alias(sunxi_serial_t *uart, const char *alias)
{
	if (alias == NULL)
		return DRIVER_ERROR_INVALID;
	return sunxi_serial_dt_read_config(uart, syterkit_dt_alias_node(alias, SUNXI_SERIAL_COMPATIBLE));
}

static inline __attribute__((always_inline)) int sunxi_serial_dt_read_stdout(sunxi_serial_t *uart)
{
