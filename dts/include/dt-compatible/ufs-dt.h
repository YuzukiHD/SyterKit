	...
	struct ufshc_config *config, int node)
{
	...
	if (!axi_clock || !cfg_clock || !controller_clock || !phy_ref_clock || !rtc_clock || !reset)
		return DRIVER_ERROR_INVALID;
	variant.axi_clk_reg = (uintptr_t)dt2c_fdt32_to_cpu(axi_clock[0]);
	variant.axi_clk_gate = dt2c_fdt32_to_cpu(axi_clock[1]);
	variant.axi_clk_src_mask = dt2c_fdt32_to_cpu(axi_clock[2]);
	variant.axi_clk_src_300 = dt2c_fdt32_to_cpu(axi_clock[3]);
	variant.axi_clk_src_200 = dt2c_fdt32_to_cpu(axi_clock[4]);
	variant.axi_clk_factor_mask = dt2c_fdt32_to_cpu(axi_clock[5]);
	variant.cfg_clk_reg = (uintptr_t)dt2c_fdt32_to_cpu(cfg_clock[0]);
	variant.cfg_clk_gate = dt2c_fdt32_to_cpu(cfg_clock[1]);
	variant.cfg_clk_src_mask = dt2c_fdt32_to_cpu(cfg_clock[2]);
	variant.cfg_clk_src_480 = dt2c_fdt32_to_cpu(cfg_clock[3]);
	variant.cfg_clk_factor_mask = dt2c_fdt32_to_cpu(cfg_clock[4]);
	variant.cfg_clk_factor = dt2c_fdt32_to_cpu(cfg_clock[5]);
	variant.rtc_xo_ctrl = (uintptr_t)dt2c_fdt32_to_cpu(rtc_clock[0]);
	variant.rtc_xo_ctrl1 = (uintptr_t)dt2c_fdt32_to_cpu(rtc_clock[1]);
	variant.rtc_wp = (uintptr_t)dt2c_fdt32_to_cpu(rtc_clock[2]);
	variant.rtc_ref_type_shift = dt2c_fdt32_to_cpu(rtc_clock[3]);
	variant.rtc_ref_type_mask = dt2c_fdt32_to_cpu(rtc_clock[4]);
	variant.rtc_external_ref_types = dt2c_fdt32_to_cpu(rtc_clock[5]);
	...
	variant.ufs_ref_clk_app_sel = dt2c_fdt32_to_cpu(controller_clock[5]);
	variant.ufs_ref_clk_app_enable = dt2c_fdt32_to_cpu(controller_clock[6]);
	variant.ufs_ref_clk_freq_mask = dt2c_fdt32_to_cpu(controller_clock[7]);
	...
	variant.phy_rst = dt2c_fdt32_to_cpu(reset[2]);
