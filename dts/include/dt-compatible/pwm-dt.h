...
static inline __attribute__((always_inline)) int sunxi_pwm_dt_read_config(sunxi_pwm_t *pwm, int node)
{
	...
	if (pwm == NULL || node < 0 || !syterkit_dt_node_available(node) ||
		dt2c_fdt_node_check_compatible(DT2C_FDT_COMPILED_TREE, node, SUNXI_PWM_COMPATIBLE) != 0)
	...
	channel_cells = (const dt2c_fdt32_t *)dt2c_fdt_getprop(
		DT2C_FDT_COMPILED_TREE, node, "allwinner,channel-config", &channel_length);
	if (reg == NULL || id_cells == NULL || pwm_cells == NULL || hosc_clock == NULL || apb_clock == NULL ||
		clock_gate == NULL || reset == NULL || channel_cells == NULL || channel_length <= 0 ||
		channel_length % (int)(SUNXI_PWM_CHANNEL_CONFIG_CELLS * sizeof(*channel_cells)) != 0)
		return DRIVER_ERROR_INVALID;
	...
	if (channel_count == 0U || channel_count > SUNXI_PWM_CHANNEL_MAX || pins == NULL ||
		dt2c_fdt32_to_cpu(pwm_cells[0]) != 3U || dt2c_fdt32_to_cpu(reg[0]) == 0U ||
		dt2c_fdt32_to_cpu(reg[1]) == 0U || dt2c_fdt32_to_cpu(id_cells[0]) > UINT8_MAX ||
		dt2c_fdt32_to_cpu(hosc_clock[0]) == 0U || dt2c_fdt32_to_cpu(apb_clock[0]) == 0U ||
		dt2c_fdt32_to_cpu(clock_gate[0]) == 0U || dt2c_fdt32_to_cpu(clock_gate[1]) >= 32U ||
		dt2c_fdt32_to_cpu(reset[0]) == 0U || dt2c_fdt32_to_cpu(reset[1]) >= 32U)
		return DRIVER_ERROR_INVALID;
	...
	for (uint32_t index = 0; index < channel_count; index++) {
		...
		if (channel >= SUNXI_PWM_CHANNEL_MAX || (channel_mask & BIT(channel)) != 0U ||
			mode > PWM_CHANNEL_BIND ||
			(mode == PWM_CHANNEL_SINGLE && (bind_channel != 0U || dead_time != 0U)) ||
			(mode == PWM_CHANNEL_BIND && (bind_channel >= SUNXI_PWM_CHANNEL_MAX ||
							     bind_channel == channel || dead_time == 0U)) ||
			!syterkit_dt_pinctrl_gpio(
				pins, (size_t)index * 3U, &gpio_controller, &config.channel[channel].pin))
			return DRIVER_ERROR_INVALID;
	...
	for (uint32_t channel = 0; channel < SUNXI_PWM_CHANNEL_MAX; channel++) {
		...
		if ((channel_mask & BIT(bind_channel)) == 0U ||
			...
			config.channel[bind_channel].dead_time != config.channel[channel].dead_time)
			return DRIVER_ERROR_INVALID;
	}
	...
	*pwm = config;
	...
	SYTERKIT_DT_TRACE("pwm config base=%p id=%u channels=0x%04x hosc=%u apb=%u gate=%p:%u reset=%p:%u\n",
		(void *)pwm->base, pwm->id, pwm->channel_mask, pwm->clk_src.clk_src_hosc, pwm->clk_src.clk_src_apb,
		(void *)pwm->pwm_clk.gate_reg_base, pwm->pwm_clk.gate_reg_offset, (void *)pwm->pwm_clk.rst_reg_base,
	...
	for (uint32_t channel = 0U; channel < SUNXI_PWM_CHANNEL_MAX; ++channel) {
		...
		SYTERKIT_DT_TRACE("pwm channel=%u mode=%u bind=%u dead_time=%u pin=%u/%u\n", channel,
			pwm->channel[channel].channel_mode, pwm->channel[channel].bind_channel,
