...
static inline __attribute__((always_inline)) void syterkit_dt_trace_bytes(
...
static inline __attribute__((always_inline)) void syterkit_dt_trace_node(const char *driver, int node)
{
	...
	dt2c_fdt_for_each_property_offset(property_offset, DT2C_FDT_COMPILED_TREE, node)
	{
		...
		pr_trace("DT:   %s.%s (%d bytes) =", driver, property_name, length);
		syterkit_dt_trace_bytes("", value, (size_t)length);
	}
}

#define SYTERKIT_DT_TRACE_NODE(driver, node) syterkit_dt_trace_node((driver), (node))
#define SYTERKIT_DT_TRACE(fmt, ...)	     pr_trace("DT: " fmt, ##__VA_ARGS__)
#else
#define SYTERKIT_DT_TRACE_NODE(driver, node) ((void)0)
#define SYTERKIT_DT_TRACE(fmt, ...)	     ((void)0)
#endif

static inline __attribute__((always_inline)) bool syterkit_dt_string_equal(
	const char *value, int length, const char *expected, size_t expected_length)
{
	return value != NULL && length == (int)expected_length + 1 && value[expected_length] == '\0' &&
	       __builtin_memcmp(value, expected, expected_length) == 0;
}

static inline __attribute__((always_inline)) const dt2c_fdt32_t *syterkit_dt_cells(
...
static inline __attribute__((always_inline)) int syterkit_dt_alias_node(const char *alias, const char *compatible)
{
	...
	if (!syterkit_dt_node_available(node) ||
		(compatible != NULL && dt2c_fdt_node_check_compatible(DT2C_FDT_COMPILED_TREE, node, compatible) != 0))
		return -DT2C_FDT_ERR_BADVALUE;
	return node;
}

static inline __attribute__((always_inline)) int syterkit_dt_read_reg_alias(
	const char *alias, uintptr_t *base, size_t *size)
{
...
static inline __attribute__((always_inline)) int syterkit_dt_phandle_node(
	int node, const char *name, const char *compatible)
{
	...
	if (!syterkit_dt_node_available(referenced) ||
		(compatible != NULL &&
			dt2c_fdt_node_check_compatible(DT2C_FDT_COMPILED_TREE, referenced, compatible) != 0))
		return -DT2C_FDT_ERR_BADVALUE;
	return referenced;
}
