...
static inline __attribute__((always_inline)) int sunxi_gic_dt_read_config(sunxi_gic_t *gic, int node)
{
	...
	if (gic == NULL || node < 0 || !syterkit_dt_node_available(node) ||
		dt2c_fdt_node_check_compatible(DT2C_FDT_COMPILED_TREE, node, SUNXI_GIC_COMPATIBLE) != 0)
	...
	if (config.distributor_base == 0U || config.distributor_size < 0x1000U || config.cpu_interface_base == 0U ||
		config.cpu_interface_size < 0x1004U || config.irq_count < 32U || config.irq_count > SUNXI_GIC_MAX_IRQS)
		return DRIVER_ERROR_INVALID;
	...
	*gic = config;
	...
	SYTERKIT_DT_TRACE("gic config distributor=%p/0x%lx cpu=%p/0x%lx irq_count=%u\n", (void *)gic->distributor_base,
		(unsigned long)gic->distributor_size, (void *)gic->cpu_interface_base,
