...
sunxi_sdhci_dt_clock_offsets_valid(uint32_t factor_n_offset,
				   uint32_t factor_m_offset) {
	...
	return (factor_n_mask & factor_m_mask) == 0U &&
	       (factor_n_mask & fixed_mask) == 0U &&
	       (factor_m_mask & fixed_mask) == 0U;
}

static inline __attribute__((always_inline)) const char *
sunxi_sdhci_dt_name(int node) {
	const char *name;
	int length;

	name = (const char *) dt2c_fdt_getprop(
			DT2C_FDT_COMPILED_TREE, node, "label", &length);
	if (name != NULL)
...
static inline __attribute__((always_inline)) bool
sunxi_sdhci_dt_dma_region(int node, uint32_t *address, uint32_t *size) {
	...
	if (region_address == 0U ||
	    region_address % __alignof__(sunxi_sdhci_desc_t) != 0U ||
...
static inline __attribute__((always_inline)) bool
sunxi_sdhci_dt_card_detect(int node, sunxi_sdhci_pinctrl_t *pinctrl) {
	...
	if (level > GPIO_LEVEL_HIGH ||
	    !sunxi_gpio_dt_read_specifier(&pinctrl->gpio_cd, cells))
		return false;
...
sunxi_sdhci_dt_read_config(sunxi_sdhci_t *sdhci, int node) {
	...
	module_clock = syterkit_dt_cells(
			node, "allwinner,module-clock",
			SUNXI_MMC_MODULE_CLOCK_CELLS);
	clock_source_rates = syterkit_dt_cells(
			node, "allwinner,clock-source-rates",
			SUNXI_SDHCI_CLOCK_SOURCE_COUNT);
	config.name = sunxi_sdhci_dt_name(node);
	if (reg == NULL || id_cells == NULL || width_cells == NULL ||
	    max_frequency == NULL || clock_gate == NULL || reset == NULL ||
	    module_clock == NULL || clock_source_rates == NULL ||
	    config.name == NULL ||
	    !sunxi_sdhci_dt_dma_region(node, &config.dma_des_addr,
				       &config.dma_des_size))
		return DRIVER_ERROR_INVALID;

	controller_id = dt2c_fdt32_to_cpu(id_cells[0]);
	bus_width = dt2c_fdt32_to_cpu(width_cells[0]);
	if (controller_id > MMC_CONTROLLER_2 ||
	    (bus_width != 1U && bus_width != 4U && bus_width != 8U) ||
	    dt2c_fdt32_to_cpu(reg[0]) == 0U ||
	    dt2c_fdt32_to_cpu(reg[1]) == 0U ||
	    dt2c_fdt32_to_cpu(max_frequency[0]) == 0U ||
	    ...
	    dt2c_fdt32_to_cpu(module_clock[0]) == 0U ||
	    !sunxi_sdhci_dt_clock_offsets_valid(
			dt2c_fdt32_to_cpu(module_clock[1]),
			dt2c_fdt32_to_cpu(module_clock[2])) ||
	    dt2c_fdt32_to_cpu(module_clock[3]) >=
			SUNXI_SDHCI_CLOCK_SOURCE_COUNT ||
	    dt2c_fdt32_to_cpu(clock_source_rates[0]) == 0U ||
	    dt2c_fdt32_to_cpu(clock_source_rates[
			dt2c_fdt32_to_cpu(module_clock[3])]) == 0U)
		return DRIVER_ERROR_INVALID;

	pin_count = bus_width + 2U + (bus_width == 8U ? 2U : 0U);
	pins = syterkit_dt_pinctrl_cells(node, (size_t) pin_count * 3U,
					&gpio_controller);
	if (pins == NULL ||
	    !syterkit_dt_pinctrl_gpio(pins, 0, &gpio_controller,
				      &config.pinctrl.gpio_clk) ||
	    !syterkit_dt_pinctrl_gpio(pins, 3, &gpio_controller,
				      &config.pinctrl.gpio_cmd) ||
	    !syterkit_dt_pinctrl_gpio(pins, 6, &gpio_controller,
				      &config.pinctrl.gpio_d0) ||
	    (bus_width >= 4U &&
	     (!syterkit_dt_pinctrl_gpio(pins, 9, &gpio_controller,
					&config.pinctrl.gpio_d1) ||
	      !syterkit_dt_pinctrl_gpio(pins, 12, &gpio_controller,
					&config.pinctrl.gpio_d2) ||
	      !syterkit_dt_pinctrl_gpio(pins, 15, &gpio_controller,
					&config.pinctrl.gpio_d3))) ||
	    (bus_width == 8U &&
	     (!syterkit_dt_pinctrl_gpio(pins, 18, &gpio_controller,
					&config.pinctrl.gpio_d4) ||
	      !syterkit_dt_pinctrl_gpio(pins, 21, &gpio_controller,
					&config.pinctrl.gpio_d5) ||
	      !syterkit_dt_pinctrl_gpio(pins, 24, &gpio_controller,
					&config.pinctrl.gpio_d6) ||
	      !syterkit_dt_pinctrl_gpio(pins, 27, &gpio_controller,
					&config.pinctrl.gpio_d7) ||
	      !syterkit_dt_pinctrl_gpio(pins, 30, &gpio_controller,
					&config.pinctrl.gpio_ds) ||
	      !syterkit_dt_pinctrl_gpio(pins, 33, &gpio_controller,
					&config.pinctrl.gpio_rst))) ||
	    !sunxi_sdhci_dt_card_detect(node, &config.pinctrl))
		return DRIVER_ERROR_INVALID;

	non_removable = dt2c_fdt_getprop(DT2C_FDT_COMPILED_TREE, node,
					  "non-removable",
					  &non_removable_length);
	if ((non_removable != NULL && non_removable_length != 0) ||
	    (non_removable == NULL &&
	     non_removable_length != -DT2C_FDT_ERR_NOTFOUND) ||
	    (non_removable != NULL && config.pinctrl.has_card_detect) ||
	    (non_removable == NULL && bus_width == 8U))
		return DRIVER_ERROR_INVALID;
	...
	config.sdhci_mmc_type = non_removable != NULL ?
			MMC_TYPE_EMMC : MMC_TYPE_SD;
	config.clk_ctrl.gate_reg_base =
			(uintptr_t) dt2c_fdt32_to_cpu(clock_gate[0]);
	config.clk_ctrl.gate_reg_offset = dt2c_fdt32_to_cpu(clock_gate[1]);
	config.clk_ctrl.rst_reg_base =
			(uintptr_t) dt2c_fdt32_to_cpu(reset[0]);
	config.clk_ctrl.rst_reg_offset = dt2c_fdt32_to_cpu(reset[1]);
	config.sdhci_clk.reg_base =
			(uintptr_t) dt2c_fdt32_to_cpu(module_clock[0]);
	config.sdhci_clk.reg_factor_n_offset =
			(uint8_t) dt2c_fdt32_to_cpu(module_clock[1]);
	config.sdhci_clk.reg_factor_m_offset =
			(uint8_t) dt2c_fdt32_to_cpu(module_clock[2]);
	config.sdhci_clk.default_clk_sel =
			(uint8_t) dt2c_fdt32_to_cpu(module_clock[3]);
	for (uint32_t source = 0U;
	     source < SUNXI_SDHCI_CLOCK_SOURCE_COUNT; ++source)
...
static inline __attribute__((always_inline)) int
sunxi_sdhci_dt_read_alias(sunxi_sdhci_t *sdhci, const char *alias) {
	if (alias == NULL)
		return DRIVER_ERROR_INVALID;
	return sunxi_sdhci_dt_read_config(
			sdhci,
			syterkit_dt_alias_node(alias, SUNXI_MMC_COMPATIBLE));
}
