...
static inline __attribute__((always_inline)) int sunxi_rtc_dt_read_config(sunxi_rtc_t *rtc, int node)
{
	...
	if (rtc == NULL || node < 0 || !syterkit_dt_node_available(node) ||
		dt2c_fdt_node_check_compatible(DT2C_FDT_COMPILED_TREE, node, SUNXI_RTC_COMPATIBLE) != 0)
	...
	if (config.data_base == 0U || config.data_size < (RTC_BOOT_INDEX + 1U) * sizeof(uint32_t) ||
