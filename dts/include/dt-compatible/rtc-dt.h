...
static inline __attribute__((always_inline)) int sunxi_rtc_dt_read_config(sunxi_rtc_t *rtc, int node)
{
	const dt2c_fdt32_t *reg;
	sunxi_rtc_t config = { 0 };

	if (rtc == NULL || node < 0 || !syterkit_dt_node_available(node) || dt2c_fdt_node_check_compatible(DT2C_FDT_COMPILED_TREE, node, SUNXI_RTC_COMPATIBLE) != 0)
	...
	if (config.data_base == 0U || config.data_size < (RTC_BOOT_INDEX + 1U) * sizeof(uint32_t) || config.data_size % sizeof(uint32_t) != 0U)
		return DRIVER_ERROR_INVALID;
...
static inline __attribute__((always_inline)) int sunxi_rtc_dt_read_alias(sunxi_rtc_t *rtc, const char *alias)
{
	int node;
