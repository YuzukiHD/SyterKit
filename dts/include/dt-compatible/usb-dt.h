...
sunxi_usb_dt_read_config(sunxi_usb_t *usb, int node) {
	...
	if (reg == NULL || id == NULL || interrupt == NULL || phy_clock == NULL ||
	    phy_reset == NULL || clock_gate == NULL || reset == NULL)
		return DRIVER_ERROR_INVALID;
	...
	if (usb_id >= SUNXI_USB_MAX_CONTROLLERS || phy_clock_gate_offset >= 32U ||
	    phy_reset_offset >= 32U || clock_gate_offset >= 32U ||
	    reset_offset >= 32U)
		return DRIVER_ERROR_INVALID;
	...
	if (config.base == 0U || config.irq == 0U ||
	    config.phy_clock_reg_base == 0U ||
	    config.clock_gate_reg_base == 0U ||
...
static inline __attribute__((always_inline)) int
sunxi_usb_dt_read_alias(sunxi_usb_t *usb, const char *alias) {
	int node;
