...
sunxi_gpio_dt_read_config(sunxi_gpio_t *gpio, int node) {
	...
	gpio_controller = dt2c_fdt_getprop(DT2C_FDT_COMPILED_TREE, node,
					   "gpio-controller", &length);
	if (reg == NULL || gpio_cells == NULL || bank_base == NULL ||
	    bank_count == NULL || gpio_controller == NULL || length != 0 ||
	    dt2c_fdt32_to_cpu(gpio_cells[0]) != SUNXI_GPIO_DT_CELLS ||
...
static inline __attribute__((always_inline)) int
sunxi_gpio_dt_read_alias(sunxi_gpio_t *gpio, const char *alias) {
	if (alias == NULL)
		return DRIVER_ERROR_INVALID;
	return sunxi_gpio_dt_read_config(
			gpio, syterkit_dt_alias_node(alias,
						   SUNXI_PINCTRL_COMPATIBLE));
}

static inline __attribute__((always_inline)) bool
sunxi_gpio_dt_read_pin(gpio_mux_t *gpio, const sunxi_gpio_t *controller,
	...
	if (port < controller->bank_base ||
	    port >= (uint32_t) controller->bank_base + controller->bank_count ||
...
static inline __attribute__((always_inline)) bool
sunxi_gpio_dt_read_specifier(gpio_mux_t *gpio,
			     const dt2c_fdt32_t *cells) {
	...
	if (gpio == NULL || cells == NULL)
		return false;
	controller_node = dt2c_fdt_node_offset_by_phandle(
...
static inline __attribute__((always_inline)) bool
sunxi_gpio_dt_read_property(gpio_mux_t *gpio, int node, const char *name) {
	if (gpio == NULL || node < 0 || name == NULL)
		return false;
	return sunxi_gpio_dt_read_specifier(
			gpio, syterkit_dt_cells(node, name,
						SUNXI_GPIO_DT_SPECIFIER_CELLS));
}
