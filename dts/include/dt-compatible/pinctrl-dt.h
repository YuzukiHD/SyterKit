...
static inline __attribute__((always_inline)) const dt2c_fdt32_t *syterkit_dt_pinctrl(int node, size_t *count, sunxi_gpio_t *controller)
{
	...
	if (controller == NULL)
		return NULL;
	pinctrl_name = (const char *)dt2c_fdt_getprop(DT2C_FDT_COMPILED_TREE, node, "pinctrl-names", &length);
	...
	if (pinctrl_cells == NULL)
		return NULL;
	pinctrl = dt2c_fdt_node_offset_by_phandle(DT2C_FDT_COMPILED_TREE, dt2c_fdt32_to_cpu(pinctrl_cells[0]));
