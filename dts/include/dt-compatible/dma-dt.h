...
static inline __attribute__((always_inline)) int sunxi_dma_dt_read_config(sunxi_dma_t *dma, int node)
{
	...
	if (reg == NULL || bus_gate == NULL || clock_gate == NULL || reset == NULL)
		return DRIVER_ERROR_INVALID;
	...
	dma->bus_clk.gate_reg_base = (uintptr_t)dt2c_fdt32_to_cpu(bus_gate[0]);
	...
	dma->dma_clk.gate_reg_base = (uintptr_t)dt2c_fdt32_to_cpu(clock_gate[0]);
	dma->dma_clk.gate_reg_offset = dt2c_fdt32_to_cpu(clock_gate[1]);
	dma->dma_clk.rst_reg_base = (uintptr_t)dt2c_fdt32_to_cpu(reset[0]);
