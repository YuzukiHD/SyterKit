...
sunxi_sid_dt_read_config(sunxi_sid_t *sid, int node) {
	...
	if (config.base == 0U || config.size < 0x204U ||
	...
	hv_switch = (const dt2c_fdt32_t *) dt2c_fdt_getprop(
			...
			"allwinner,efuse-hv-switch", &hv_switch_length);
	if (hv_switch == NULL) {
		...
		if (hv_switch_length != (int) sizeof(*hv_switch))
			return DRIVER_ERROR_INVALID;
		config.efuse_hv_switch =
			(uintptr_t) dt2c_fdt32_to_cpu(hv_switch[0]);
		if (config.efuse_hv_switch == 0U ||
		    (config.efuse_hv_switch &
...
static inline __attribute__((always_inline)) int
sunxi_sid_dt_read_alias(sunxi_sid_t *sid, const char *alias) {
	int node;
