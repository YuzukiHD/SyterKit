...
#ifndef __DT_COMPATIBLE_SPI_NOR_DT_H__
#define __DT_COMPATIBLE_SPI_NOR_DT_H__
...
static inline __attribute__((always_inline)) int spi_nor_dt_read_config(spi_nor_t *nor, int node, sunxi_spi_t *spi)
{
	...
	if (nor == NULL || spi == NULL || node < 0 || !syterkit_dt_node_available(node) ||
	...
	*nor = config;
	...
	SYTERKIT_DT_TRACE("spi-nor config spi=%p chip_select=%u max_frequency=%u\n", (void *)nor->spi, nor->chip_select,
		nor->max_frequency);
	return DRIVER_OK;
}

static inline __attribute__((always_inline)) int spi_nor_dt_read_alias(
	spi_nor_t *nor, const char *alias, sunxi_spi_t *spi)
{
	int node;
