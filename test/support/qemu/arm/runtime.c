...
static void qemu_print_unsigned(uintptr_t value, unsigned int radix, unsigned int width, char padding)
{
...
static void qemu_vprintf(const char *format, va_list args)
{
	while (*format) {
		...
		case 'p':
			qemu_puts("0x");
			qemu_print_unsigned((uintptr_t)va_arg(args, void *), 16U, 2U * sizeof(uintptr_t), '0');
			break;
		case 's':
			qemu_puts(va_arg(args, const char *));
			break;
		case 'u':
			qemu_print_unsigned(va_arg(args, unsigned int), 10U, width, padding);
			break;
		case 'x':
		case 'X':
			qemu_print_unsigned(va_arg(args, unsigned int), 16U, width, padding);
			break;
		default:
			qemu_putchar('%');
