...
TEST_CPPFLAGS := -I$(srctree)/arch/riscv/include -I$(TEST_ROOT)/include \
	-I$(srctree)/include -DXCFG_FORMAT_FLOAT=0 \
...
TEST_CFLAGS := -std=gnu11 $(TEST_OPT) -g -Wall -Wextra -Werror -ffreestanding \
	...
	-march=rv64gc_zicsr -mabi=lp64d -mcmodel=medany \
	-msmall-data-limit=0 $(TEST_EXTRA_CFLAGS)
TEST_AFLAGS := $(TEST_CFLAGS)
TEST_LDFLAGS := -nostdlib -nostartfiles -Wl,--gc-sections \
	-Wl,--build-id=none -Wl,-z,noexecstack -Wl,-melf64lriscv \
...
run: $(TEST_ELF)
	@command -v $(QEMU_RISCV64) >/dev/null || { \
		echo "test: $(QEMU_RISCV64) is required" >&2; exit 1; }
	@echo "  QEMU    $(CASE_NAME)"
	@set +e; timeout $(QEMU_TIMEOUT) $(QEMU_RISCV64) \
