...
void test_case_main(const char *case_dir) {
	...
	TEST_EQ(DRIVER_ERROR_INVALID,
		 spi_nor_dt_read_alias(&rejected, "nor0", &spi1));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 spi_nor_dt_read_alias(&rejected, "nor-disabled", &spi0));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 spi_nor_dt_read_alias(&rejected, "nor-bad-cs", &spi0));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 spi_nor_dt_read_alias(&rejected, "nor-bad-frequency", &spi0));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 spi_nor_dt_read_alias(&rejected, "nor-wrong-compatible", &spi0));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 spi_nor_dt_read_alias(&rejected, "missing", &spi0));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 spi_nor_dt_read_alias(NULL, "nor0", &spi0));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 spi_nor_dt_read_alias(&rejected, NULL, &spi0));
