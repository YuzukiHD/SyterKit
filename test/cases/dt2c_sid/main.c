...
static uint32_t sram_read_offsets[128];
static size_t sram_read_count;
...
uint32_t test_mmio_read32(uintptr_t address)
{
	...
	if (address == 0x1060U) {
		efuse_read_count++;
	} else if (address >= 0x1200U && address < 0x1400U) {
		if (sram_read_count < sizeof(sram_read_offsets) / sizeof(sram_read_offsets[0]))
			sram_read_offsets[sram_read_count] = (uint32_t)(address - 0x1200U);
		sram_read_count++;
	}
...
void test_case_main(const char *case_dir)
{
	...
	TEST_EQ(0x1200U, sid0.sram_base);
	...
	TEST_EQ(0x4000U, sid1.sram_base);
	...
	TEST_EQ(DRIVER_ERROR_INVALID, sunxi_sid_dt_read_alias(&rejected, "sid-bad-sram"));
	...
	last_mmio_read = 0U;
	TEST_EQ(0x12345678U, sunxi_efuse_sram_read(&sid0, 0x3cU));
	TEST_EQ(0x123cU, last_mmio_read);
	last_mmio_read = 0U;
	TEST_EQ(0x12345678U, sunxi_efuse_sram_read(&sid1, 0x3cU));
	TEST_EQ(0x403cU, last_mmio_read);
	last_mmio_read = 0U;
	TEST_EQ(0U, sunxi_efuse_sram_read(&sid0, 1U));
	TEST_EQ(0U, last_mmio_read);
	TEST_EQ(0U, sunxi_efuse_sram_read(&sid0, 0x200U));
	TEST_EQ(0U, last_mmio_read);
	TEST_EQ(0U, sunxi_efuse_sram_read(&wrapped, 0U));
	TEST_EQ(0U, last_mmio_read);
	TEST_EQ(0U, sunxi_efuse_sram_read(NULL, 0U));
	last_mmio_read = 0U;
	efuse_read_count = 0U;
	TEST_EQ(0x12345678U, sunxi_efuse_read(&sid0, 0x3cU));
	TEST_EQ(0x1060U, last_mmio_read);
	TEST_EQ(1U, efuse_read_count);
	...
	TEST_EQ(0U, efuse_read_count);
	TEST_EQ(expected_read_count, sram_read_count);
	TEST_ASSERT(expected_read_count <= sizeof(sram_read_offsets) / sizeof(sram_read_offsets[0]));
	if (expected_read_count > sizeof(sram_read_offsets) / sizeof(sram_read_offsets[0]))
		return;
	...
	for (size_t section = 0; section < sunxi_sid_section_count; section++) {
		...
		for (size_t index = 0; index < word_count; index++)
			TEST_EQ(entry->offset + index * sizeof(uint32_t), sram_read_offsets[expected_read_count++]);
	}
}
