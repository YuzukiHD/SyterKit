...
uint32_t test_mmio_read32(uintptr_t address) {
	...
	if (address == 0x1060U) {
		if (efuse_read_count <
		    sizeof(efuse_read_offsets) / sizeof(efuse_read_offsets[0]))
			efuse_read_offsets[efuse_read_count] =
					current_efuse_offset;
		efuse_read_count++;
	}
...
void test_mmio_write32(uintptr_t address, uint32_t value) {
	if (address == 0x1040U && (value & 0x3U) == 0x2U)
		...
		if (hv_switch_write_count <
		    sizeof(hv_switch_values) / sizeof(hv_switch_values[0]))
...
void test_case_main(const char *case_dir) {
	...
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_sid_dt_read_alias(&rejected, "sid-disabled"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_sid_dt_read_alias(&rejected, "sid-short"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_sid_dt_read_alias(&rejected, "sid-bad-switch"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_sid_dt_read_alias(&rejected, "sid-wrong-compatible"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_sid_dt_read_alias(&rejected, "missing"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_sid_dt_read_alias(&rejected, NULL));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_sid_dt_read_alias(NULL, "sid0"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_sid_dt_read_config(&rejected, -1));
	...
	for (size_t section = 0; section < sunxi_sid_section_count; section++)
		expected_read_count +=
				(sunxi_sid_sections[section].size_bits + 31U) / 32U;
	TEST_EQ(64U, expected_read_count);
	TEST_EQ(expected_read_count, efuse_read_count);
	TEST_ASSERT(expected_read_count <=
		    sizeof(efuse_read_offsets) / sizeof(efuse_read_offsets[0]));
	if (expected_read_count >
	    sizeof(efuse_read_offsets) / sizeof(efuse_read_offsets[0]))
		return;
	...
	for (size_t section = 0; section < sunxi_sid_section_count; section++) {
		...
		for (size_t index = 0; index < word_count; index++)
			TEST_EQ(entry->offset + index * sizeof(uint32_t),
				 efuse_read_offsets[expected_read_count++]);
	}
}
