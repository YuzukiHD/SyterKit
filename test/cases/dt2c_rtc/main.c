...
void test_case_main(const char *case_dir) {
	...
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_rtc_dt_read_alias(&rejected, "rtc-disabled"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_rtc_dt_read_alias(&rejected, "rtc-short"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_rtc_dt_read_alias(&rejected, "rtc-unaligned"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_rtc_dt_read_alias(&rejected, "rtc-wrong-compatible"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_rtc_dt_read_alias(&rejected, "missing"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_rtc_dt_read_alias(&rejected, NULL));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_rtc_dt_read_alias(NULL, "rtc0"));
