...
void test_case_main(const char *case_dir) {
	...
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_alias(&rejected, "intc-disabled"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_alias(&rejected, "intc-short-dist"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_alias(&rejected, "intc-short-cpu"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_alias(&rejected, "intc-too-few-irqs"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_alias(&rejected, "intc-too-many-irqs"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_alias(&rejected, "intc-wrong-compatible"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_alias(&rejected, NULL));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_alias(&rejected, "missing"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_alias(NULL, "intc0"));
	TEST_EQ(DRIVER_ERROR_INVALID,
		 sunxi_gic_dt_read_config(&rejected, -1));
