...
void test_boot(void) {
	...
	qemu_puts("TEST START backtrace_arm_invalid_context_qemu\n");
	bad_pc = backtrace((char *) 4, valid_sp, (char *) 8);
	bad_sp = backtrace((char *) valid_pc_anchor, (long *) 4,
			   (char *) valid_pc_anchor);
	...
	if (bad_pc == 0 && bad_sp == 0 && bad_context == 0) {
		...
		qemu_puts("TEST PASS backtrace_arm_invalid_context_qemu\n");
		qemu_exit(1);
	}
	qemu_puts("TEST FAIL backtrace_arm_invalid_context_qemu\n");
	qemu_exit(0);
}
