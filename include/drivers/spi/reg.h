...
/**
 * @file reg.h
 * @brief Register map for the Allwinner SPI controller.
 */
...
#define SPI_GC_TP_EN \
	(0x1 << 7) /* SPI transmit stop enable 1:stop transmit data when RXFIFO is full; 0:ignore RXFIFO status; default:1 */
...
#define SPI_TC_SPOL \
	(0x1 << 2) /* SPI Chip select signal polarity control,default: 1,low effective like this:~~|_____~~ */
#define SPI_TC_SSCTL \
	(0x1 << 3) /* SPI chip select control,default 0:SPI_SSx remains asserted between SPI bursts,1:negate SPI_SSx between SPI bursts */
...
#define SPI_TC_DHB \
	(0x1 << 8) /* Discard Hash Burst,default 0:receiving all spi burst in BC period 1:discard unused,fectch WTC bursts */
...
#define SPI_TC_XCH \
	(0x1 << 31) /* Exchange burst default 0:idle,1:start exchange;when BC is zero,this bit cleared by SPI controller*/
...
#define SPI_INTEN_RX_RDY \
	(0x1 << 0) /* rxFIFO Ready Interrupt Enable,---used for immediately received,0:disable;1:enable */
...
#define SPI_INTEN_SSI \
	(0x1 << 13) /* SSI interrupt Enable,chip select from valid state to invalid state,for slave used only */
...
#define SPI_WAIT_SWC_MASK \
	(0xF << 16) /* used only in master mode: Wait before start dual data transfer in dual SPI mode */
