...
typedef struct {
	...
	int (*load_buffer)(struct sunxi_remoteproc *remoteproc, const void *firmware, size_t size);
...
typedef struct sunxi_remoteproc {
	...
	sunxi_remoteproc_address_map_t address_map[SUNXI_REMOTEPROC_MAX_ADDRESS_MAPS];
	size_t address_map_count;
	sunxi_remoteproc_register_t registers[SUNXI_REMOTEPROC_MAX_REGISTERS];
