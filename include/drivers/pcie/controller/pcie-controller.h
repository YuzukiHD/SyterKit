...
#define PCIE_BDF(bus, device, function)                                             \
	((((uint32_t)(bus) & 0xffU) << 16) | (((uint32_t)(device) & 0x1fU) << 11) | \
...
#define PCIE_BDF_CONFIG_TARGET(bdf) \
	((PCIE_BDF_BUS(bdf) << 24) | (PCIE_BDF_DEVICE(bdf) << 19) | (PCIE_BDF_FUNCTION(bdf) << 16))
...
struct pcie_controller_ops {
	...
	int (*read_dbi)(struct pcie_controller *controller, uint32_t offset, uint8_t size,
		uint32_t *value); /**< Read from DBI space. */
	int (*write_dbi)(struct pcie_controller *controller, uint32_t offset, uint8_t size,
		uint32_t value); /**< Write to DBI space. */
	int (*dbi_ro_write_enable)(struct pcie_controller *controller,
		bool enable); /**< Enable or disable DBI read-only register writes. */
	int (*set_ep_bar)(struct pcie_controller *controller, uint8_t function, uint8_t bar, bool enable,
		bool bar_64bit); /**< Configure an endpoint BAR. */
	...
	int (*set_mode)(
		struct pcie_controller *controller, enum pcie_mode mode); /**< Set the controller operating mode. */
	...
	int (*disable_atu)(struct pcie_controller *controller, enum pcie_atu_direction direction,
		uint8_t index); /**< Disable an ATU window. */
};
