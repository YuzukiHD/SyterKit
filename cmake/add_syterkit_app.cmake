...
function(add_syterkit_app target_name)
    ...
    target_link_libraries(${target_name}_fel ${APP_COMMON_LIBRARY} -T"${LINK_SCRIPT_FEL}" -flto -nostdlib -Wl,-z,noexecstack,-Map,${target_name}_fel.map)
    ...
    target_link_libraries(${target_name}_bin ${APP_COMMON_LIBRARY} -T"${LINK_SCRIPT_BIN}" -flto -nostdlib -Wl,-z,noexecstack,-Map,${target_name}_bin.map)
    ...
    if(CMAKE_BUILD_TYPE STREQUAL Trace OR CMAKE_BUILD_TYPE STREQUAL Debug)
        add_custom_command(
            TARGET ${target_name}_bin
            POST_BUILD COMMAND ${CROSS_COMPILE}objdump -S ${target_name}_bin > ${target_name}_bin.asm
            COMMENT "Objdump ${target_name}_bin"
        )

        add_custom_command(
            TARGET ${target_name}_fel
            POST_BUILD COMMAND ${CROSS_COMPILE}objdump -S ${target_name}_fel > ${target_name}_fel.asm
