...
void set_pll_cpux_axi(void) {
    ...
    /* disable pll */
    reg_val = read32(CCU_PLL_CPUX_CTRL_REG) & ~(1 << 31);
    writel(reg_val, CCU_PLL_CPUX_CTRL_REG);

    /* set default val: clk is 1008M, PLL_OUTPUT = 24 M*N / (M*P) */
    reg_val = (read32(CCU_PLL_CPUX_CTRL_REG) & ~((0x3 << 16) | (0xff << 8) | (0x3 << 0))) | (41 << 8);
    writel(reg_val, CCU_PLL_CPUX_CTRL_REG);

    /* lock enable */
    reg_val = read32(CCU_PLL_CPUX_CTRL_REG) | (1 << 29);
    writel(reg_val, CCU_PLL_CPUX_CTRL_REG);
    ...
    while (!(read32(CCU_PLL_CPUX_CTRL_REG) & (0x1 << 28)))
        ;
    sdelay(20);
    ...
    reg_val = (read32(CCU_CPUX_AXI_CFG_REG) & ~(0x03 << 24)) | (0x03 << 24);
...
void set_pll_periph0(void) {
    ...
    if ((1U << 31) & read32(CCU_PLL_PERI0_CTRL_REG)) {
    ...
    /* lock enable */
    reg_val = read32(CCU_PLL_PERI0_CTRL_REG);
    reg_val |= (1 << 29);
...
void set_ahb(void) {
    ...
    writel((0x03 << 24) | read32(CCU_PSI_AHB1_AHB2_CFG_REG), CCU_PSI_AHB1_AHB2_CFG_REG);
    sdelay(1);
    /*PLL6:AHB3 = 600M:200M*/
    writel((2 << 0) | (0 << 8), CCU_AHB3_CFG_GREG);
    writel((0x03 << 24) | read32(CCU_AHB3_CFG_GREG), CCU_AHB3_CFG_GREG);
}

void set_apb(void) {
    /*PLL6:APB1 = 600M:100M */
    writel((2 << 0) | (1 << 8), CCU_APB1_CFG_GREG);
    writel((0x03 << 24) | read32(CCU_APB1_CFG_GREG), CCU_APB1_CFG_GREG);
    sdelay(1);
}
...
void set_pll_dma(void) {
    ...
    writel(read32(CCU_DMA_BGR_REG) | (1 << 0), CCU_DMA_BGR_REG);
}
...
void set_pll_mbus(void) {
    ...
    /* set MBUS div */
    reg_val = read32(CCU_MBUS_CFG_REG);
    reg_val |= (2 << 0);
...
int sunxi_clock_init_gpadc(void) {
    uint32_t reg_val = 0;
    /* reset */
    reg_val = read32(CCU_GPADC_BGR_REG);
    reg_val &= ~(1 << 16);
...
void sunxi_clk_reset(void) {
    ...
    reg_val = read32(CCU_PSI_AHB1_AHB2_CFG_REG);
    reg_val &= (~((0x3 << 24) | (0x3 << 8) | (0x3)));
    writel(reg_val, CCU_PSI_AHB1_AHB2_CFG_REG);

    reg_val = read32(CCU_APB1_CFG_GREG);
    reg_val &= (~((0x3 << 24) | (0x3 << 8) | (0x3)));
...
void sunxi_clk_dump() {
    ...
    /* PLL CPU */
    reg32 = read32(CCU_CPUX_AXI_CFG_REG);
    ...
    reg32 = read32(CCU_PLL_CPUX_CTRL_REG);
    p0 = (reg32 >> 16) & 0x03;
    if (p0 == 0) {
        p1 = 1;
    } else if (p0 == 1) {
    ...
    /* PLL PERIx */
    reg32 = read32(CCU_PLL_PERI0_CTRL_REG);
    if (reg32 & (1 << 31)) {
        plln = ((reg32 >> 8) & 0xff) + 1;
        pllm = (reg32 & 0x01) + 1;
        p0 = ((reg32 >> 16) & 0x03) + 1;
