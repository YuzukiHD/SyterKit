...
static int ccu_set_pll_ddr_clk(int index, dram_para_t *para) {
	...
	if (((para->dram_tpr13 >> 6) & 0x1) == index) pll_clk = (para->dram_clk << 1);
	else
...
static void mctl_com_init(dram_para_t *para) {
	...
	if ((para->dram_para2 & (1 << 8)) && ((para->dram_para2 & 0xf000) != 0x1000)) width = 32;// 32-bit width if dual-rank is configured
	else
...
static int init_DRAM(int type, dram_para_t *para) {
	...
	if (para->dram_tpr13 & (1 << 28)) {
		if ((readl(SUNXI_R_CPUCFG_SUP_STAN_FLAG) & (1 << 16)) || dramc_simple_wr_test(mem_size_mb, 4096)) return 0;
	}
