...
static inline void set_ahb(void) {
    ...
    writel((0x03 << 24) | readl(CCU_BASE + CCU_PSI_AHB1_AHB2_CFG_REG),
           CCU_BASE + CCU_PSI_AHB1_AHB2_CFG_REG);
    udelay(1);
    /*PLL6:AHB3 = 600M:200M*/
...
void sunxi_clk_init(void)  {
    printk(LOG_LEVEL_DEBUG, "Set SoC 1855 (A133/R818) CLK Start.\n");
    set_platform_config();
    set_pll_cpux_axi();
    set_pll_periph0();
    set_ahb();
    ...
    set_pll_mbus();
    set_modules_clock();
    printk(LOG_LEVEL_DEBUG, "set pll end\n");
    return;
}
...
void sunxi_clk_dump() {
    ...
    printk(LOG_LEVEL_DEBUG, "CLK: CPU PLL=%s FREQ=%luMHz\r\n", clock_str, ((((reg32 >> 8) & 0xff) + 1) * 24 / p1));
    ...
    if (reg32 & (1 << 31)) {
        ...
        printk(LOG_LEVEL_DEBUG, "CLK: PLL_PERI0 (2X)=%luMHz, (1X)=%luMHz, (1200M)=%luMHz\r\n", (24 * plln) / (pllm * p0),
    ...
    if (reg32 & (1 << 31)) {
        ...
        printk(LOG_LEVEL_DEBUG, "CLK: PLL_PERI1 (2X)=%luMHz, (1X)=%luMHz, (1200M)=%luMHz\r\n", (24 * plln) / (pllm * p0),
               (24 * plln) / (pllm * p0) >> 1, (24 * plln) / (pllm * p1));
    } else {
