...
_add_dts_size:
		...
		const char *updated_bootargs_str = fdt_getprop(image.of_dest, bootargs_node, "bootargs", &len);
		...
		/* Get bootargs string */
		const char *bootargs_str = fdt_getprop(image.of_dest, bootargs_node, "bootargs", &len);
