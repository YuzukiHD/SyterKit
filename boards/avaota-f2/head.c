...
/**
 * @file head.c
 * @brief Boot0 file header definition for the Avaota F2 (sun252iw1).
 */
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
typedef struct boot_file_head {
	...
	uintptr_t ret_addr; /* the return value */ /**< Return address used after boot. */
...
/*
 * The BROM jumps to the first 64-byte-aligned payload after the boot head.
 * sizeof() depends on the ISA (0x30 on RV32, 0x40 on RV64 because of the
 * pointer fields), so round it up to 64 when encoding the jump offset.
 */
...
#define BOOT_RET_ADDR ((uintptr_t)__efex_result_start)
#else
#define BOOT_RET_ADDR ((uintptr_t)__code_start_address)
#endif
