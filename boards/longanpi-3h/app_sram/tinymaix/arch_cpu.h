...
{
	*result = sptr[0] * kptr[0] + sptr[1] * kptr[1] + sptr[2] * kptr[2] + sptr[3] * kptr[3] + sptr[4] * kptr[4] +
		  sptr[5] * kptr[5] + sptr[6] * kptr[6] + sptr[7] * kptr[7] + sptr[8] * kptr[8];
	return;
}
...
TM_INLINE void tm_postprocess_sum(
	sumtype_t sum, btype_t b, int act, mtype_t *outp, sctype_t scale, sctype_t out_s, zptype_t out_zp)
{ //printf("sum=%.6f,", sum);
...
TM_INLINE void tm_postprocess_sum(
	int n, sumtype_t *sums, btype_t *bs, int act, mtype_t *outp, sctype_t *scales, sctype_t out_s, zptype_t out_zp)
{
	for (int i = 0; i < n; i++) {
...
#if !TM_FASTSCALE
TM_INLINE void tm_postprocess_sum(int n, sumtype_t *sums, btype_t *bs, int act, mtype_t *outp, sctype_t *scales,
	sctype_t out_s_inv, zptype_t out_zp)
#else
TM_INLINE void tm_postprocess_sum(
	int n, sumtype_t *sums, btype_t *bs, int act, mtype_t *outp, int32_t *scales, int32_t out_s, zptype_t out_zp)
#endif
{
	for (int i = 0; i < n; i++) {
		sumtype_t sum = sums[i];
		...
		outp[i] =
			(mtype_t)(sumf * out_s_inv +
				  out_zp); //(mtype_t)((int)(sumf/out_s) + out_zp) //(mtype_t)((int)(sumf/out_s +0.5) + out_zp)
#else
