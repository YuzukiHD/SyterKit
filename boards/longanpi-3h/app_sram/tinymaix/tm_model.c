...
tm_err_t TM_WEAK tm_run(tm_mdl_t *mdl, tm_mat_t *in, tm_mat_t *out)
{
	...
	for (mdl->layer_i = 0; mdl->layer_i < mdl->b->layer_cnt; mdl->layer_i++) {
		...
		case TML_DWCONV2D: {
			...
			res = tml_conv2d_dwconv2d(&_in, &_out, (wtype_t *)(mdl->layer_body + l->w_oft),
				(btype_t *)(mdl->layer_body + l->b_oft), l->kernel_w, l->kernel_h, l->stride_w,
				l->stride_h, l->dilation_w, l->dilation_h, l->act, l->pad[0], l->pad[1], l->pad[2],
				l->pad[3], l->depth_mul, (sctype_t *)(mdl->layer_body + l->ws_oft), h->in_s, h->in_zp,
				h->out_s, h->out_zp);
			break;
		}
		case TML_GAP: {
		...
		case TML_FC: {
			...
			res = tml_fc(&_in, &_out, (wtype_t *)(mdl->layer_body + l->w_oft),
				(btype_t *)(mdl->layer_body + l->b_oft), (sctype_t *)(mdl->layer_body + l->ws_oft),
				h->in_s, h->in_zp, h->out_s, h->out_zp);
			break;
		}
		case TML_SOFTMAX: {
