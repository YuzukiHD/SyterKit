...
tm_err_t tm_stat(tm_mdlbin_t *b) {
	...
	for (layer_i = 0; layer_i < b->layer_cnt; layer_i++) {
		...
		TM_DBG("type=%d, is_out=%d, size=%d, in_oft=%d, out_oft=%d, in_dims=[%d,%d,%d,%d], out_dims=[%d,%d,%d,%d], in_s=%.3f, in_zp=%d, out_s=%.3f, out_zp=%d\n", h->type,
			   h->is_out, h->size, h->in_oft, h->out_oft, h->in_dims[0], h->in_dims[1], h->in_dims[2], h->in_dims[3], h->out_dims[0], h->out_dims[1], h->out_dims[2],
			   h->out_dims[3], h->in_s, (int32_t) (h->in_zp), h->out_s, (int32_t) (h->out_zp));
		if (h->type < TML_MAXCNT) {
			...
			switch (h->type) {
				case TML_CONV2D: {
					...
					TM_DBG("Conv2d: kw=%d, kh=%d, sw=%d, sh=%d, dw=%d, dh=%d, act=%d, pad=[%d,%d,%d,%d], dmul=%d, ws_oft=%d, w_oft=%d, b_oft=%d\n", l->kernel_w, l->kernel_h,
						   l->stride_w, l->stride_h, l->dilation_w, l->dilation_h, l->act, l->pad[0], l->pad[1], l->pad[2], l->pad[3], l->depth_mul, l->ws_oft, l->w_oft, l->b_oft);
					break;
				}
				case TML_GAP:
			...
			sum_ops += ops;
			TM_PRINTF("%03d\t%s      \t%3d,%3d,%3d\t%d\t%d\t%d\t%ld\t", layer_i, tml_str_tbl[h->type], h->out_dims[1], h->out_dims[2], h->out_dims[3], h->in_oft, h->out_oft,
					  h->size - tml_headsize_tbl[h->type], (long int) (memout * sizeof(mtype_t)));
			TM_PRINTF("%d\r\n", ops);
		} else {
