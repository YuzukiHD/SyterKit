...
//1x1 pw conv
TM_INLINE tm_err_t l_tml_pwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy,
	int dx, int dy, int act, int pad_top, int pad_bottom, int pad_left, int pad_right, int dmul, sctype_t *ws,
	sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp)
{
...
//normal conv
TM_INLINE tm_err_t l_tml_conv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy,
	int dx, int dy, int act, int pad_top, int pad_bottom, int pad_left, int pad_right, int dmul, sctype_t *ws,
	sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp)
{
	int pad_flag = (pad_top != 0 || pad_bottom != 0 || pad_left != 0 || pad_right != 0);
...
//dwconv
TM_INLINE tm_err_t l_tml_dwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy,
	int dx, int dy, int act, int pad_top, int pad_bottom, int pad_left, int pad_right, int dmul, sctype_t *ws,
	sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp)
{
	int pad_flag = (pad_top != 0 || pad_bottom != 0 || pad_left != 0 || pad_right != 0);
			...
			if (!slow_flag) { //valid or same valid part
				...
				if (maxk == 9) {
					for (int c = 0; c < cho; c++) {
						...
						tm_postprocess_sum(
							1, &sum, b + c, act, outp, SUMSCALE, OUTSCALE, out_zp);
						outp++;
						sptr += 1;
					}
				} else {
...
//dwconv
TM_INLINE tm_err_t l_tml_dwconv2d_3x3_part(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx,
	int sy, int dx, int dy, int act, int pad_top, int pad_bottom, int pad_left, int pad_right, int dmul,
	sctype_t *ws, sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp, int x0, int x1, int y0, int y1)
{
	int pad_flag = (pad_top != 0 || pad_bottom != 0 || pad_left != 0 || pad_right != 0);
	int maxk = kw * kh;
	int chi = in->c;
...
// 40ms->27ms
TM_INLINE tm_err_t l_tml_dwconv2d_3x3_nostride(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh,
	int sx, int sy, int dx, int dy, int act, int pad_top, int pad_bottom, int pad_left, int pad_right, int dmul,
		...
		for (int x = 0; x + CONV_BLK_STEPX <= out->w; x += CONV_BLK_STEPX) {
			...
			if (!slow_flag) { //valid or same valid part
				...
				for (int c = 0; c < cho; c++) {
					...
					tm_postprocess_sum(
						1, &sum0, b + c, act, outp + 0 * cho, SUMSCALE, OUTSCALE, out_zp);
					tm_postprocess_sum(
						1, &sum1, b + c, act, outp + 1 * cho, SUMSCALE, OUTSCALE, out_zp);
					tm_postprocess_sum(1, &sum2, b + c, act, outp + (out->w + 0) * cho, SUMSCALE,
						OUTSCALE, out_zp);
					tm_postprocess_sum(1, &sum3, b + c, act, outp + (out->w + 1) * cho, SUMSCALE,
						OUTSCALE, out_zp);
					outp++;
					sptr++;
				}
...
#error "unsupport mdl type"
	...
	if (out->w % CONV_BLK_W != 0 || out->h % CONV_BLK_H != 0) { //cal rest part
		...
		l_tml_dwconv2d_3x3_part(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left,
			pad_right, dmul, ws, in_s, in_zp, out_s, out_zp, out->w - out->w % CONV_BLK_W, out->w, 0,
			out->h / CONV_BLK_H * CONV_BLK_H);
		l_tml_dwconv2d_3x3_part(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left,
			pad_right, dmul, ws, in_s, in_zp, out_s, out_zp, 0, out->w, out->h - out->h % CONV_BLK_H,
			out->h / CONV_BLK_H * CONV_BLK_H);
	}
...
TM_INLINE tm_err_t l_tml_mdwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy,
	int dx, int dy, int act, int pad_top, int pad_bottom, int pad_left, int pad_right, int dmul, sctype_t *ws,
	sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp)
{
	int pad_flag = (pad_top != 0 || pad_bottom != 0 || pad_left != 0 || pad_right != 0);
...
tm_err_t tml_conv2d_dwconv2d(tm_mat_t *in, tm_mat_t *out, wtype_t *w, btype_t *b, int kw, int kh, int sx, int sy,
	int dx, int dy, int act, int pad_top, int pad_bottom, int pad_left, int pad_right, int dmul, sctype_t *ws,
	sctype_t in_s, zptype_t in_zp, sctype_t out_s, zptype_t out_zp) //kernel: (cho, chi, h, w)
{
	TM_PERF_INIT(t_conv);
	...
	if (maxk == 1) {
		...
		l_tml_pwconv2d(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right,
			dmul, ws, in_s, in_zp, out_s, out_zp);
		TM_PERF_ADD(t_pwconv);
	} else if (dmul == 0) {
		TM_PERF_START(t_conv); //normal conv
		l_tml_conv2d(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right, dmul,
		...
		if (kh == 3 && kw == 3) { //opt for 3x3 dwconv
			if (sx == 1 && sy == 1) {
				l_tml_dwconv2d_3x3_nostride(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top,
					pad_bottom, pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp);
			} else {
				l_tml_dwconv2d_3x3_part(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom,
					pad_left, pad_right, dmul, ws, in_s, in_zp, out_s, out_zp, 0, out->w, 0,
					out->h);
			}
		} else {
			l_tml_dwconv2d(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left,
				pad_right, dmul, ws, in_s, in_zp, out_s, out_zp);
		}
		TM_PERF_ADD(t_dwconv);
	} else { //dmul>1 dwconv, rare
		l_tml_mdwconv2d(in, out, w, b, kw, kh, sx, sy, dx, dy, act, pad_top, pad_bottom, pad_left, pad_right,
			dmul, ws, in_s, in_zp, out_s, out_zp);
	}
